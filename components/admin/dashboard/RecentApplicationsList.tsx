  ...
  return (
    <div className="divide-y divide-slate-50">
      {items.slice(0, 8).map((app) => {
        const name =
          [app.first_name, app.last_name].filter(Boolean).join(' ') || app.full_name || 'Unknown';
        return (
          <Link
            key={app.id}
            href={app.href}
          ...
          >
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${app.urgent ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600'}`}
            >
              {name[0].toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-slate-400 mt-0.5 truncate">
                ...
                <span
                  className={`ml-2 font-medium ${app.urgent ? 'text-rose-600' : 'text-slate-400'}`}
                >
