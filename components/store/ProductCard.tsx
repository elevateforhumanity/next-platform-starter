...
export function ProductCard({ product, variant = 'default' }: ProductCardProps) {
      ...
      <div className="group bg-white rounded-xl border hover:shadow-lg transition-all duration-300 overflow-hidden flex">
        ...
        <div className="flex-1 p-5 flex flex-col">
          <div className="flex items-start justify-between mb-2">
            <div>
              <h3 className="font-bold text-lg text-slate-900 group-hover:text-brand-blue-600 transition">
                {product.name}
              </h3>
          ...
          <div className="flex items-center gap-4 mb-3">
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4, 5].map((i) => (
                <Star
                  key={i}
                  className={`w-4 h-4 ${i <= Math.floor(product.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-slate-700'}`}
                />
              ))}
          ...
          <div className="mt-auto flex items-center justify-between">
            <div>
              ...
              {product.originalPrice && (
                <span className="text-slate-700 line-through text-sm ml-2">
                  ${product.originalPrice}
                </span>
              )}
              <span className="text-slate-700 text-sm">/mo</span>
            ...
            <div className="flex gap-2">
              <Link
                ...
                className="px-4 py-2 text-slate-900 font-medium hover:bg-slate-100 rounded-lg transition"
              >
                Demo
              </Link>
              <Link
  ...
  // Default variant
  return (
    <div className="group bg-white rounded-2xl border-2 border-slate-100 overflow-hidden hover:border-brand-blue-200 hover:shadow-2xl transition-all duration-300">
      {/* Image */}
      ...
      {/* Content */}
      <div className="p-6">
        <h3 className="text-xl font-bold text-slate-900 mb-1 group-hover:text-brand-blue-600 transition">
          {product.name}
        </h3>
