...
export type LessonType = 'video' | 'pdf' | 'scorm' | 'quiz' | 'reflection' | 'link' | 'other';
  ...
  return (
    <div className="grid gap-4 md:grid-cols-[minmax(0,2fr),minmax(0,1fr)]">
      <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
        <p className="text-[10px] uppercase tracking-wide text-brand-orange-400">Course</p>
        <h1 className="mt-1 text-[13px] font-semibold text-slate-900">{courseTitle}</h1>
        {activeLesson ? (
          <>
            <p className="mt-2 text-[11px] font-semibold text-slate-200">{activeLesson.title}</p>
            <div className="mt-2 rounded-lg border border-slate-800 bg-white p-2 text-[11px]">
              ...
              {activeLesson.contentType === 'quiz' && (
                <p className="text-slate-600">
                  Quiz content will appear here. We can wire this to live quiz items stored in
                  Supabase.
                </p>
              )}
              {activeLesson.contentType === 'reflection' && (
                <p className="text-slate-600">
                  Reflection prompt: We&apos;ll store learner responses in Supabase so coaches can
                  review them.
                </p>
              )}
              {activeLesson.contentType === 'link' && activeLesson.contentUrl && (
                <a
              ...
              {activeLesson.contentType === 'other' && (
                <p className="text-slate-600">
                  This lesson uses a custom content type that we can define later.
                </p>
              )}
            </div>
            <p className="mt-2 text-[10px] text-slate-500">
              Later, this area will mark the lesson as completed and sync progress to the database.
            </p>
          </>
        ) : (
          <p className="mt-2 text-[11px] text-slate-600">No lessons found for this course yet.</p>
        )}
      </div>

      <aside className="rounded-xl border border-slate-800 bg-slate-950 p-3 text-[11px]">
        <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">Lessons</p>
