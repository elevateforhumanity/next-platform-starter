    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-brand-red-600 text-white text-center py-2 px-4 text-xs font-semibold flex items-center justify-center gap-3">
        <span className="hidden sm:inline">
          Video Walkthrough — no real data, no account required
        </span>
        <span className="sm:hidden">Interactive Demo</span>
        <Link
          href="/store/demos"
          className="underline underline-offset-2 opacity-90 hover:opacity-100 flex items-center gap-1"
        >
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 h-14">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden sm:flex items-center gap-1.5 text-slate-400 text-xs">
              ...
              <span
                className={`${meta.accent} text-white text-[11px] font-bold px-2.5 py-0.5 rounded-full`}
              >
                {meta.badge}
              </span>
            </span>
          ...
          <nav className="flex-1 py-2">
            {nav.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon
                    className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-brand-red-600' : 'text-slate-400'}`}
                  />
                  {item.label}
                </Link>
              );
