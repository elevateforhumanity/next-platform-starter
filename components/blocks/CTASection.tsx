...
export default function CTASection({ heading, subheading, ctas, bg = 'dark' }: Props) {
  const sectionBg =
    bg === 'red'
      ? 'bg-brand-red-600 border-t border-brand-red-700'
      : bg === 'white'
        ? 'bg-white border-t-4 border-slate-900'
  ...
  return (
    <section className={`${sectionBg} py-16 sm:py-20 px-4`}>
      <div className="max-w-3xl mx-auto text-center">
        <h2 className={`text-2xl sm:text-3xl font-extrabold ${headingColor} mb-3 leading-tight`}>
          {heading}
        </h2>
        {subheading && (
          <p className={`${subColor} text-base mb-8 max-w-xl mx-auto`}>{subheading}</p>
        )}
