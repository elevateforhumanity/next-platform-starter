/**
 * Generates a signed MOU PDF for nail_technician salon apprenticeship partners.
...
export async function generateNailMOUPdf(data: NailMOUPDFData): Promise<Uint8Array> {
  ...
  // ── Header ──────────────────────────────────────────────────────────────────
  // Purple header bar for nail_technician (vs dark navy for barber)
  page.drawRectangle({
    x: 0,
    y: pageHeight - 90,
    width: pageWidth,
  ...
  y -= 16;
  page.drawText('Indiana Nail Technician Apprenticeship Program', {
    x: margin,
    y,
    size: 11,
    font: regularFont,
  ...
  y -= 12;
  page.drawText(
    `Version ${data.mou_version || '2025-nail_technician-01'}  ·  Effective: ${signedDate}`,
  ...
  const sections = [
    {
      title: '1. Purpose',
      body: `This MOU establishes the terms under which ${data.salon_name} ("Employer") will serve as a training site for the DOL Registered Nail Technician Apprenticeship Program sponsored by ${SPONSOR} ("Sponsor"), RAPIDS Program No. ${RAPIDS}, governed by 29 CFR Parts 29 and 30.`,
    },
    {
      title: '2. Employer Responsibilities',
      body: `The Employer agrees to: (a) Provide 2,000 hours of On-the-Job Learning (OJL) per apprentice; (b) Assign a licensed Indiana nail technician as supervisor (Indiana IPLA license, 2+ years experience); (c) Maintain a current Indiana nail salon license and all required IPLA permits throughout the term; (d) Pay apprentices per the progressive wage schedule in the Standards of Apprenticeship; (e) Submit monthly OJL hour reports to the Sponsor; (f) Carry workers' compensation insurance covering the apprentice; (g) Comply with 29 CFR Part 30 equal opportunity requirements; (h) Notify Sponsor within 5 business days of any apprentice status change; (i) Allow DOL and Sponsor audits of the training site.`,
    },
    {
    ...
    },
    {
      ...
      body: `Primary Supervising Nail Technician: ${data.supervisor_name || 'As designated by Employer'}. Indiana IPLA Nail Technician License: ${data.supervisor_license || 'On file'}. The supervising nail technician must hold a current Indiana nail_technician license with at least 2 years of experience throughout the term of this MOU.`,
    },
    {
