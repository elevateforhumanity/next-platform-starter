...
export async function generateTppSurveyPdf(data: TppSurveyData): Promise<Uint8Array> {
  ...
  txt('Elevate for Humanity Technical and Career Institute', hx, H - 30, bold, 11, C.headerText);
  txt('8888 Keystone Crossing, Suite 1300  ·  Indianapolis, IN 46240  ·  (317) 314-3757', hx, H - 44, regular, 8, rgb(0.75, 0.85, 1));
  txt('elevate4humanityedu@gmail.com  ·  elevateforhumanity.org', hx, H - 56, regular, 8, rgb(0.75, 0.85, 1));
  ...
  const addRunningHeader = (pg: PDFPage, pageNum: number) => {
    ...
    pg.drawText('Elevate for Humanity  ·  FSSA SNAP E&T TPP Application', { x: hx2, y: H - 20, size: 8, font: bold, color: C.headerText });
  ...
  for (const comp of components) {
    ...
    if (!comp.offered) {
      txt('This component is not offered by Elevate for Humanity at this time.', ML + 6, y, italic, 8.5, C.gray);
      y -= 16;
      continue;
    }
  ...
  allPages.forEach((pg, i) => {
    ...
    pg.drawText('Elevate for Humanity Technical and Career Institute  ·  8888 Keystone Crossing, Suite 1300, Indianapolis, IN 46240  ·  (317) 314-3757', {
