...
export function runDiagnostics(ret: Return1040): Diagnostic[] {
  ...
  if (ssnDigits.length !== 9 || ssnDigits === '000000000') {
    out.push({
      code: 'TP_SSN_INVALID',
      severity: 'error',
      message: 'Taxpayer SSN must be 9 digits and cannot be all zeros.',
      field: 'taxpayer.ssn',
    });
  }
  ...
  if (!ret.taxpayer.firstName?.trim() || !ret.taxpayer.lastName?.trim()) {
    out.push({
      code: 'TP_NAME_MISSING',
      severity: 'error',
      message: 'Taxpayer first and last name are required.',
      field: 'taxpayer',
    });
  }
  ...
  if (!ret.taxpayer.dob) {
    out.push({
      code: 'TP_DOB_MISSING',
      severity: 'error',
      message: 'Taxpayer date of birth is required.',
      field: 'taxpayer.dob',
    });
  }
  ...
  if (ret.filingStatus === 'mfj' && !ret.spouse) {
    out.push({
      ...
      severity: 'error',
      message: 'MFJ filing status requires spouse data.',
      field: 'spouse',
    });
  }
  ...
  if (ret.filingStatus === 'mfs' && !ret.spouse?.ssn) {
    out.push({
      code: 'SPOUSE_SSN_REQUIRED_MFS',
      severity: 'warning',
      message: 'MFS returns should include spouse SSN.',
      field: 'spouse.ssn',
    });
  }
  ...
  if (ret.spouse) {
    ...
    if (spouseSsn.length !== 9 || spouseSsn === '000000000') {
      out.push({
        code: 'SPOUSE_SSN_INVALID',
        severity: 'error',
        message: 'Spouse SSN is invalid.',
        field: 'spouse.ssn',
      });
    }
    if (ret.spouse.ssn === ret.taxpayer.ssn) {
      out.push({
        code: 'SSN_DUPLICATE',
        severity: 'error',
        message: 'Taxpayer and spouse SSN cannot be the same.',
        field: 'spouse.ssn',
      });
    }
  }
  ...
  for (const [i, dep] of ret.dependents.entries()) {
    ...
    if (depSsn.length !== 9 || depSsn === '000000000') {
      out.push({
        code: 'DEP_SSN_INVALID',
        severity: 'error',
        message: `Dependent ${i + 1} SSN is invalid.`,
        field: `dependents[${i}].ssn`,
      });
    }
    if (depSsns.includes(depSsn)) {
      out.push({
        code: 'DEP_SSN_DUPLICATE',
        severity: 'error',
        message: `Dependent ${i + 1} SSN is a duplicate.`,
        field: `dependents[${i}].ssn`,
      });
    }
    depSsns.push(depSsn);
    ...
    if (!dep.relationship) {
      out.push({
        code: 'DEP_RELATIONSHIP_MISSING',
        severity: 'error',
        message: `Dependent ${i + 1} relationship is required.`,
        field: `dependents[${i}].relationship`,
      });
    }
  }
  ...
  if (ret.w2s.length === 0 && ret.forms1099.length === 0 && !ret.schedules.scheduleC) {
    out.push({
      code: 'NO_INCOME_DOCS',
      severity: 'warning',
      message: 'No income documents present (W-2, 1099, or Schedule C).',
      field: 'w2s',
    });
  }
  ...
  for (const [i, w2] of ret.w2s.entries()) {
    if (!w2.ein || !/^\d{2}-\d{7}$/.test(w2.ein)) {
      out.push({
        code: 'W2_EIN_INVALID',
        severity: 'error',
        message: `W-2 ${i + 1}: employer EIN format invalid (expected XX-XXXXXXX).`,
        field: `w2s[${i}].ein`,
      });
    }
    if (w2.wages < 0) {
      out.push({
        code: 'W2_WAGES_NEGATIVE',
        severity: 'error',
        message: `W-2 ${i + 1}: wages cannot be negative.`,
        field: `w2s[${i}].wages`,
      });
    }
    if (w2.federalWithholding > w2.wages) {
      out.push({
        ...
        severity: 'warning',
        message: `W-2 ${i + 1}: federal withholding exceeds wages — verify.`,
        field: `w2s[${i}].federalWithholding`,
      });
    }
  }
  ...
  if (ret.schedules.scheduleC) {
    ...
    if (sc.grossReceipts < 0) {
      out.push({
        code: 'SCHC_GROSS_NEGATIVE',
        severity: 'error',
        message: 'Schedule C gross receipts cannot be negative.',
        field: 'schedules.scheduleC.grossReceipts',
      });
    }
    if (!sc.principalBusinessCode || !/^\d{6}$/.test(sc.principalBusinessCode)) {
      out.push({
        code: 'SCHC_BUSINESS_CODE_INVALID',
        severity: 'error',
        message: 'Schedule C principal business code must be 6 digits.',
        field: 'schedules.scheduleC.principalBusinessCode',
      });
    }
  }
  ...
  if (ret.schedules.eitc) {
    if (ret.schedules.eitc.investmentIncome > 11600) {
      ...
      out.push({
        code: 'EITC_INVESTMENT_INCOME_EXCEEDS_LIMIT',
        severity: 'error',
        message: 'Investment income exceeds EITC limit for 2024 ($11,600).',
        field: 'schedules.eitc.investmentIncome',
      });
    }
    if (ret.preparer && !ret.preparer.selfPrepared) {
      out.push({
        code: 'EITC_FORM_8867_REQUIRED',
        severity: 'warning',
        message: 'Form 8867 (EITC Due Diligence) is required for paid preparers claiming EITC.',
        field: 'schedules.eitc',
      });
    }
  }
  ...
  if (!ret.address.street || !ret.address.city || !ret.address.state || !ret.address.zip) {
    out.push({
      ...
      severity: 'error',
      message: 'Taxpayer address is incomplete.',
      field: 'address',
    });
  }
  ...
  if (!ret.taxpayerSignature?.pin) {
    out.push({
      ...
      severity: 'error',
      message: 'Taxpayer signature PIN is required before filing.',
      field: 'taxpayerSignature',
    });
  }

  if (ret.filingStatus === 'mfj' && !ret.spouseSignature?.pin) {
    out.push({
      code: 'SPOUSE_SIGNATURE_MISSING',
      severity: 'error',
      message: 'Spouse signature PIN is required for MFJ returns.',
      field: 'spouseSignature',
    });
  }
  ...
  if (ret.preparer && !ret.preparer.selfPrepared) {
    if (!ret.preparer.ptin || !/^P\d{8}$/.test(ret.preparer.ptin)) {
      out.push({
        code: 'PTIN_INVALID',
        severity: 'error',
        message: 'Preparer PTIN must be in format P########.',
        field: 'preparer.ptin',
      });
    }
  }
  ...
  if (ret.taxpayerSignature?.pin && ret.priorYearAGI === undefined) {
    out.push({
      code: 'PRIOR_YEAR_AGI_MISSING',
      severity: 'warning',
      message:
        'Prior year AGI is recommended for self-select PIN identity verification. Use 0 for first-time filers.',
      field: 'priorYearAGI',
    });
  }
  ...
  if (ret.metadata.status === 'draft') {
    out.push({
      code: 'RETURN_IN_DRAFT',
      severity: 'info',
      message: 'Return is in draft status and cannot be transmitted.',
