...
export async function fetchOnetOccupation(socCode: string): Promise<OnetOccupation> {
  ...
  const techSkills = await onetGet<any>(`/online/occupations/${code}/details/technology_skills`).catch(() => ({ category: [] }));
  await sleep(1100);

  const education = await onetGet<any>(`/online/occupations/${code}/details/education_training_experience`).catch(() => null);
  ...
  return {
    ...
    education: {
      typical_level: education?.education?.typical_level?.name ?? 'High school diploma or equivalent',
