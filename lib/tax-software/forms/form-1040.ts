...
export function calculateForm1040(taxReturn: TaxReturn): Form1040Result {
  ...
  const other1099 =
    (taxReturn.form1099MISC?.reduce((sum, f) => sum + (f.otherIncome || 0), 0) || 0) +
...
function calculateTax(taxableIncome: number, filingStatus: string): number {
  const brackets =
    TAX_BRACKETS_2024[filingStatus as keyof typeof TAX_BRACKETS_2024] || TAX_BRACKETS_2024.single;
...
function calculateAdditionalChildTaxCredit(
  ...
  const scheduleC =
    taxReturn.scheduleCBusiness?.reduce((sum, b) => sum + Math.max(0, b.netProfit), 0) || 0;
