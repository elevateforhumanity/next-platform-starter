...
export function calculateForm1040(taxReturn: TaxReturn): Form1040Result {
  ...
  // Line 2: Interest
  const line2a =
    taxReturn.form1099INT?.reduce((sum, f) => sum + (f.taxExemptInterest || 0), 0) || 0;
  ...
  const line12 =
    taxReturn.deductionType === 'itemized' && itemizedDeduction > standardDeduction
      ? itemizedDeduction
      : standardDeduction;
  ...
  const { childTaxCredit, otherDependentCredit } = calculateChildTaxCredit(
    taxReturn,
    config,
    line11,
  );
  ...
  const line29 = calculateAdditionalChildTaxCredit(
    taxReturn,
    config,
    childTaxCredit,
    line18,
...
function getStandardDeduction(taxReturn: TaxReturn, config: TaxConfig): number {
  let deduction =
    config.standardDeductions[taxReturn.filingStatus] || config.standardDeductions.single;
...
function calculateChildTaxCredit(
  ...
  const threshold =
    ctcConfig.phaseOutThreshold[taxReturn.filingStatus] || ctcConfig.phaseOutThreshold.single;
...
function calculateAdditionalChildTaxCredit(
  ...
  const refundableAmount =
    (earnedIncome - ctcConfig.earnedIncomeThreshold) * ctcConfig.refundableRate;
...
function calculateEITC(
  ...
  const investmentIncome =
    (taxReturn.form1099INT?.reduce((sum, f) => sum + f.interestIncome, 0) || 0) +
  ...
  const numChildren = Math.min(
    taxReturn.dependents?.filter((d) => d.childTaxCredit).length || 0,
    3,
  );
...
function calculateSelfEmploymentTax(netSelfEmployment: number, config: TaxConfig): number {
  ...
  const ssTax =
    Math.min(taxableAmount, config.socialSecurity.wageBase) *
    config.socialSecurity.selfEmployedRate;
...
function calculateAdditionalMedicareTax(
  ...
  const threshold =
    config.medicare.additionalTaxThreshold[taxReturn.filingStatus] ||
...
function calculateNIIT(
  ...
  const threshold = config.niit.threshold[taxReturn.filingStatus] || config.niit.threshold.single;
...
function calculateAMT(
  ...
  const phaseOutThreshold =
    amtConfig.phaseOutThreshold[taxReturn.filingStatus] || amtConfig.phaseOutThreshold.single;
  ...
  if (amtTaxableIncome <= amtConfig.rate2Threshold) {
    ...
    amtTax =
      amtConfig.rate2Threshold * amtConfig.rate1 +
...
function calculateQBIDeduction(
  ...
  const thresholdStart =
    qbiConfig.thresholdStart[taxReturn.filingStatus] || qbiConfig.thresholdStart.single;
  const thresholdEnd =
    qbiConfig.thresholdEnd[taxReturn.filingStatus] || qbiConfig.thresholdEnd.single;
