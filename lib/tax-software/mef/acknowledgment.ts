...
export const REJECTION_CODES: Record<
  string,
  { category: string; description: string; resolution: string }
> = {
  'IND-031': {
    category: 'Identity',
    description: 'Primary SSN has already been used on a return',
    resolution:
      'Verify SSN is correct. If duplicate filing suspected, file Form 14039 Identity Theft Affidavit',
  },
  'IND-032': {
    category: 'Identity',
...
export class AcknowledgmentHandler {
  ...
  async processAcknowledgment(acknowledgment: MeFAcknowledgment): Promise<void> {
    ...
    await setAuditContext(this.supabase, {
      systemActor: 'mef_acknowledgment',
      requestId: acknowledgment.submissionId,
    });
    ...
    await this.supabase.from('mef_acknowledgments').upsert({
  ...
  private async handleRejectedReturn(acknowledgment: MeFAcknowledgment): Promise<void> {
    ...
    if (acknowledgment.errors) {
      for (const error of acknowledgment.errors) {
        await this.supabase.from('mef_errors').insert({
          submission_id: acknowledgment.submissionId,
  ...
  async canResubmit(submissionId: string): Promise<boolean> {
    ...
    return (
      ...
      ((submissionData.resubmission_count as number) || 0) < 5
    );
  }
  ...
  getResolution(errorCode: string): string {
    return (
      REJECTION_CODES[errorCode]?.resolution ||
