...
export function validateTaxReturn(taxReturn: TaxReturn): ValidationResult {
  ...
  if (
    !taxReturn.address.street ||
    !taxReturn.address.city ||
    !taxReturn.address.state ||
    !taxReturn.address.zip
  ) {
    errors.push({
      errorCode: 'F1040-003',
      errorCategory: 'reject',
  ...
  if (taxReturn.directDeposit) {
    if (
      ...
      taxReturn.directDeposit.routingNumber.length !== 9
    ) {
      errors.push({
        errorCode: 'F1040-050',
        errorCategory: 'reject',
    ...
    if (
      ...
      taxReturn.directDeposit.accountNumber.length < 4
    ) {
      errors.push({
        errorCode: 'F1040-051',
        errorCategory: 'reject',
  ...
  if (taxReturn.itemizedDeductions) {
    const saltTotal =
      (taxReturn.itemizedDeductions.stateLocalTaxes || 0) +
