...
export async function runCosmetologyPostPayment(
  ...
  if (!enrollmentId) {
    ...
    if (!profile?.id) {
      logger.warn(
        '[cosmetology-post-payment] No profile found for paid application — enrollment deferred',
        { studentEmail },
      );
  ...
  try {
    ...
    const onboardingUrl = `${siteUrl}/programs/cosmetology-apprenticeship/orientation`;
    const dashboardUrl = `${siteUrl}/apprentice`;
    const loginUrl = `${siteUrl}/login`;

    await sendEmail({
      to: studentEmail,
      subject: "You're enrolled — start your Cosmetology Apprenticeship onboarding",
      html: `
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#1a1a1a;">
  <div style="background:#1e293b;padding:24px 32px;border-radius:8px 8px 0 0;">
    ...
    <p style="margin:4px 0 0;color:#94a3b8;font-size:13px;">Cosmetology Apprenticeship Program</p>
  </div>
  <div style="padding:32px;background:#fff;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 8px 8px;">
    ...
    logger.error('[cosmetology-post-payment] Student email failed (non-fatal)', err);
