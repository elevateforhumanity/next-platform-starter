...
export default async function MyDashboardPage() {
  ...
  const { data: profile } = await db
    .from('profiles')
    .select(
      'id, first_name, last_name, full_name, role, email, avatar_url, enrollment_status, onboarding_completed',
    )
    .eq('id', user.id)
    .maybeSingle();
    ...
    tabs.includes('trades')
      ? (async () => {
          ...
          const { data: partnerLink } = await db
            .from('partner_users')
            .select(
              'partner_id, status, partners(id, partner_type, onboarding_completed, mou_signed, approval_status)',
            )
            .eq('user_id', user.id)
            .maybeSingle();
