    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <ChevronRight
            className={`w-5 h-5 text-slate-400 transition-transform flex-shrink-0 ${expanded ? 'rotate-90' : ''}`}
          />
        </div>
      </button>
          ...
          {!showAnswer ? (
            <div className="space-y-3">
              ...
              {scenario.diagnosticSteps.slice(0, currentStep + 1).map((step, i) => (
                <div
                  key={i}
                  className={`rounded-lg p-4 border ${step.isKeyFinding ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-slate-200'}`}
                >
...
export default function HVACClassroomPreview({
  ...
  const modId = isCareerSafe
    ? 'careersafe'
    : mod.id || `hvac-${String(currentModuleIndex + 1).padStart(2, '0')}`;
      ...
      <div className="flex h-screen bg-white">
        ...
        <div className="fixed top-0 left-0 right-0 z-[60] bg-brand-blue-700 text-white text-center py-3 text-sm font-medium shadow-lg">
          ...
          <span className="hidden sm:inline text-slate-600">
            640 Hours &middot; EPA 608 &middot; OSHA 10 &middot; CPR/AED
          </span>
          <Link
            href="/apply?program=hvac-technician"
            className="ml-3 bg-brand-blue-600 hover:bg-brand-blue-700 text-white px-3 py-1 rounded-full text-xs font-bold transition"
          >
            Apply Now
          </Link>
        </div>
        ...
        {/* Sidebar */}
        <aside
          className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 w-80 bg-white border-r overflow-y-auto transition-transform duration-300 fixed md:relative h-full z-40 pt-10`}
        >
          <div className="p-5 border-b">
            <Link
              href="/programs/hvac-technician"
              className="text-sm text-brand-blue-600 hover:underline flex items-center gap-1"
            >
            ...
            <div className="mt-3">
              <div className="flex justify-between text-xs text-slate-500 mb-1">
                <span>
                  {completedModuleCount}/{course.modules.length} modules
                </span>
          ...
          <div className="p-3">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm font-medium truncate ${locked ? 'text-slate-400' : 'text-slate-800'}`}
                      >
                        {m.title}
                      </p>
            ...
            <div className="mt-4 pt-4 border-t border-slate-200">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 px-3">
                External Certifications
              </p>
              <button
                onClick={() => {
                  setCurrentModuleIndex(course.modules.length);
                  setSidebarOpen(false);
                }}
                className={`w-full text-left p-3 rounded-lg mb-1 transition ${
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm font-medium truncate ${currentModuleIndex === course.modules.length ? 'text-amber-800' : 'text-slate-800'}`}
                    >
          ...
          <div className="max-w-4xl mx-auto px-4 md:px-8 py-6">
            ...
            {currentModuleIndex === course.modules.length ? (
              <>
                <div className="mb-6">
                  ...
                  <h1 className="text-2xl md:text-3xl font-bold text-slate-900 mt-1">
                    CareerSafe — OSHA 10 &amp; CPR/AED
                  </h1>
                  <p className="text-slate-600 mt-2 text-sm">
                    These credentials are delivered through CareerSafe&apos;s online platform.
                    Elevate provides access — CareerSafe issues the DOL cards.
                  </p>
                </div>
              ...
              <>
                {/* Module header */}
                <div className="mb-6">
                  <p className="text-sm text-brand-blue-600 font-medium">
                  ...
                  {mod.competency && (
                    <p className="text-slate-600 mt-2 text-sm">{mod.competency}</p>
                  )}
                </div>
                ...
                {activeTab === 'video' && (
                  <div>
                    ...
                    {currentModuleIndex !== 0 && !isTabDone('video') && (
                      <button
                        onClick={() => completeTab('video')}
                        className="mt-4 w-full py-3 bg-brand-green-600 text-white font-semibold rounded-lg hover:bg-brand-green-700 transition"
                      >
                        Mark Video Complete — Continue to Content
                      </button>
                    )}
                    {isTabDone('video') && (
                      <p className="mt-4 flex items-center gap-2 text-brand-green-600 text-sm font-medium">
                ...
                {activeTab === 'content' && (
                  <div className="space-y-8">
                    ...
                    {(() => {
                      const diagrams = visualLibrary.filter((d: any) =>
                        d.moduleIds?.includes(modId),
                      );
                      const interactiveDiagrams = diagrams.filter(
                        (d) => d.hasInteractive && DIAGRAM_COMPONENTS[d.id],
                      );
                      const staticDiagrams = diagrams.filter(
                        (d) => !d.hasInteractive || !DIAGRAM_COMPONENTS[d.id],
                      );
                      ...
                      return (
                        <div className="space-y-4">
                          ...
                          <p className="text-sm text-slate-600">
                            Click on components in each diagram to learn what they do. These same
                            diagrams appear throughout the program — repetition builds
                            understanding.
                          </p>
                    ...
                    {!isTabDone('content') && (
                      <button
                        onClick={() => completeTab('content')}
                        className="mt-6 w-full py-3 bg-brand-green-600 text-white font-semibold rounded-lg hover:bg-brand-green-700 transition"
                      >
                    ...
                    {isTabDone('content') && (
                      <p className="mt-4 flex items-center gap-2 text-brand-green-600 text-sm font-medium">
                        <CheckCircle className="w-4 h-4" /> Content complete
                      </p>
                    )}
                  </div>
                )}
                  ...
                  <div>
                    ...
                    {['hvac-04', 'hvac-09', 'hvac-10', 'hvac-11', 'hvac-12'].includes(modId) && (
                      <div className="space-y-6 mb-8">
                        ...
                        <TroubleshootScenario
                          ...
                          onComplete={(result: { correct: boolean }) => {
                            if (result.correct) completeTab('lab');
                          }}
                        />
                      </div>
                    )}
                    ...
                    <div className="space-y-6">
                      ...
                      {(() => {
                        ...
                        const quizDiagrams = diagrams.filter(
                          (d) => d.hasInteractive && DIAGRAM_COMPONENTS[d.id],
                        );
                        ...
                        return (
                          <div className="space-y-4">
                            ...
                            <p className="text-sm text-slate-600">
                              Identify the components in each diagram. Click on each part to reveal
                            ...
                            {quizDiagrams.map((d) => {
                              ...
                              const quizComponents: Record<string, React.ReactNode> = {
                                ...
                                'troubleshooting-flowchart': (
                                  <TroubleshootingFlowchart mode="guided" />
                                ),
                              };
                              return quizComponents[d.id] ? (
                                <div key={d.id}>{quizComponents[d.id]}</div>
                              ) : null;
                            })}
                          </div>
                        );
                      ...
                      {(() => {
                        const scenarios = serviceScenarios.filter((s: any) =>
                          s.moduleIds?.includes(modId),
                        );
                        ...
                        return (
                          <div className="space-y-4">
                            ...
                            <p className="text-sm text-slate-600">
                              Work through real diagnostic scenarios. Follow the steps a technician
                              would take on an actual service call.
                            </p>
                      ...
                      {mod.lessons.filter((l) => l.type === 'lab').length === 0 &&
                        visualLibrary
                          .filter((d: any) => d.moduleIds?.includes(modId))
                          .filter((d) => d.hasInteractive).length === 0 &&
                        serviceScenarios.filter((s: any) => s.moduleIds?.includes(modId)).length ===
                          0 && (
                          <div className="bg-white border border-slate-200 rounded-lg p-6 text-center">
                            <FlaskConical className="w-10 h-10 text-slate-300 mx-auto mb-2" />
                            <p className="text-slate-500">
                              This module&apos;s lab is completed during OJT (on-the-job training).
                            </p>
                          </div>
                        )}
                      {!isTabDone('lab') && (
                        <button
                          onClick={() => completeTab('lab')}
                          className="mt-4 w-full py-3 bg-brand-green-600 text-white font-semibold rounded-lg hover:bg-brand-green-700 transition"
                        >
                    ...
                    {isTabDone('lab') && (
                      <p className="mt-4 flex items-center gap-2 text-brand-green-600 text-sm font-medium">
                        <CheckCircle className="w-4 h-4" /> Lab complete
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="flex items-center justify-between border-t border-slate-200 pt-6 pb-12 mt-8">
                  ...
                  >
                    {!allTabsDone ? (
                      <>
                        <Lock className="w-4 h-4" /> Complete All Tabs
                      </>
                    ) : (
                      <>
