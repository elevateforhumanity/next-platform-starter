...
export const metadata: Metadata = {
  title: 'Skilled Trades Portal — Elevate for Humanity',
  description: 'Your skilled trades training portal. Track HVAC, CDL, Welding, Electrical, and other trades program progress.',
...
export default async function TradesPortalPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect('/login?redirect=/portal/trades');
  ...
  return (
    <IndustryPortalPage
      ...
      industryIcon={<Wrench className="w-5 h-5 text-white" />}
      accentColor="text-amber-600"
      accentBg="bg-amber-600"
      userName={profile?.full_name ?? user.email ?? 'Student'}
      enrolledPrograms={enrolledPrograms}
      availablePrograms={[
        { title: 'HVAC Technician', slug: 'hvac-technician', credential: 'EPA Section 608', progress: 0, status: 'not_started' },
        { title: 'CDL Training', slug: 'cdl-training', credential: 'Commercial Driver License', progress: 0, status: 'not_started' },
        { title: 'Welding', slug: 'welding', credential: 'AWS Certification', progress: 0, status: 'not_started' },
        { title: 'Construction Trades', slug: 'construction-trades', credential: 'NCCER / OSHA', progress: 0, status: 'not_started' },
      ]}
