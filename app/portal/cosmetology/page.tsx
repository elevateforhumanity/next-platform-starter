import { Metadata } from 'next';
import { ApprenticeshipProgramDashboard } from '@/components/apprenticeship/ApprenticeshipProgramDashboard';
import { loadApprenticeshipDashboard } from '@/lib/apprenticeship/load-apprenticeship-dashboard';

export const dynamic = 'force-dynamic';
export const metadata: Metadata = {
  title: 'Cosmetology Apprenticeship Dashboard',
  description: 'Track your cosmetology apprenticeship hours, competencies, RTI lessons, and training progress.',
  robots: { index: false, follow: false },
};

export default async function CosmetologyPortalPage() {
  const data = await loadApprenticeshipDashboard('cosmetology-apprenticeship');
  return <ApprenticeshipProgramDashboard {...data} />;
}
