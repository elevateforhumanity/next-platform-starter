...
export const GET = withAuth(
  async (req: NextRequest) => {
    ...
    try {
      ...
      (pageViews ?? []).forEach((v) => {
        pageViewsByPath[v.path] = (pageViewsByPath[v.path] || 0) + 1;
      });
      ...
      (conversions ?? []).forEach((c) => {
        ...
        conversionValueByType[c.conversion_type] =
          (conversionValueByType[c.conversion_type] || 0) + parseFloat(c.value || '0');
      });
      ...
      (conversions ?? []).forEach((c) => {
        ...
        dailyConversions[d] = (dailyConversions[d] || 0) + 1;
      });
      ...
      return NextResponse.json({
        data: {
          ...
          conversionsByType: Object.entries(conversionsByType).map(([type, count]) => ({
            type,
            count,
