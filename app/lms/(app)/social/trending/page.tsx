...
export default async function TrendingPage() {
  ...
  try {
    const { data } = await supabase
      .from('social_posts')
      .select(
        'id, title, content, like_count, comment_count, share_count, created_at, profiles!social_posts_author_id_fkey(full_name)',
      )
  ...
  try {
    const { data } = await supabase
      .from('forum_threads')
      .select(
        'id, title, forum_id, reply_count, view_count, created_at, profiles!forum_threads_author_id_fkey(full_name)',
      )
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        {hasContent ? (
                ...
                <div className="bg-white rounded-xl shadow-sm border">
                  ...
                  <div className="divide-y">
                    {trendingThreads.map((thread: any) => (
                      <Link
                        key={thread.id}
                        href={`/lms/forums/${thread.forum_id}/threads/${thread.id}`}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-amber-50 rounded-xl p-6">
                ...
                <p className="text-sm text-slate-700 mb-4">
                  Browse our training programs and start your career journey.
                </p>
                <Link
                  href="/lms/courses"
          ...
          <div className="bg-white rounded-xl shadow-sm border p-12 text-center">
            ...
            <Link
              href="/lms/social"
              className="inline-flex items-center gap-2 px-6 py-3 bg-brand-orange-600 text-white rounded-lg hover:bg-brand-orange-700 font-medium"
            >
              Go to Social Feed
            </Link>
          </div>
