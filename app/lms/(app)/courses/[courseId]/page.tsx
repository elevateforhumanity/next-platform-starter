...
export default async function CoursePage({ params }: { params: Params }) {
  ...
  const modulesQuery = db
    .from('course_modules')
    .select('id, title, order_index, is_draft, available_from')
    .eq('course_id', lessonCourseId)
  ...
  const { data: lessonsRaw } = await db
    .from('lms_lessons')
    .select(
      'id, title, duration_minutes, order_index, content_type, step_type, module_id, activities, lesson_slug, passing_score',
    )
    .eq('course_id', lessonCourseId)
    .eq('status', 'published')
    .order('order_index', { ascending: true });
  ...
  const checkpointCount = allLessons.filter(
    (l) => l.step_type === 'checkpoint' || l.content_type === 'quiz',
  ).length;
  ...
  const activePhase = PHASE_MAP.find(
    (p) => activeModuleOrder >= p.range[0] && activeModuleOrder <= p.range[1],
  );

  // Lesson position within active module
  const activeLessonsInModule = activeModule?.lessons ?? [];
  const completedInActiveModule = activeLessonsInModule.filter(
        ...
        <div className="relative h-[180px] sm:h-[220px] w-full overflow-hidden bg-slate-900">
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-5 sm:p-8 max-w-5xl mx-auto w-full">
            <nav className="flex items-center gap-1.5 text-xs text-white/40 mb-2">
              <Link href="/lms/courses" className="hover:text-white transition">
                My Courses
              </Link>
              ...
              <div className="flex-1">
                ...
                {completedCount > 0 && (
                  <p className="text-xs text-slate-400 mt-1">
                    {completedModules > 0
                      ? `${completedModules} module${completedModules !== 1 ? 's' : ''} complete · `
                      : ''}
              ...
              <div className="flex items-baseline justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    ...
                    {totalHours > 0 ? `${totalHours}+ hours` : `${totalMinutes}m`}
                  </p>
                </div>
                {nextLesson && (
                  <Link
                ...
                <>
                  ...
                  {PHASE_MAP.map((phase, phaseIdx) => {
                    const phaseModules = modules.filter(
                      (_: any, i: number) => i + 1 >= phase.range[0] && i + 1 <= phase.range[1],
                    );
                    ...
                    const phaseComplete = phaseModules.every(
                      (mod: any) =>
                        ...
                        mod.lessons.every((l: any) => progressMap.get(l.id)?.completed),
                    );
                    ...
                    const activePhaseIdx = PHASE_MAP.findIndex(
                      (p) => p.label === activePhase?.label,
                    );
                    const isLocked = !isActivePhase && !phaseComplete && phaseIdx > activePhaseIdx;

                    const phaseStatus: 'current' | 'completed' | 'locked' = phaseComplete
                      ? 'completed'
                      : isLocked
                        ? 'locked'
                        : 'current';
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    {
                      label: 'Current phase',
                      value: activePhase ? `${activePhase.label}: ${activePhase.name}` : '—',
                    },
                    {
                      label: 'Hours remaining',
                      value:
                        totalHours > 0
                          ? `~${Math.round((totalMinutes * (1 - progressPct / 100)) / 60)}h`
                          : '—',
                    },
                  ].map(({ label, value }) => (
      ...
      <div className="relative h-[340px] sm:h-[440px] w-full overflow-hidden bg-slate-900">
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-6 sm:p-12 max-w-5xl mx-auto w-full">
          ...
          <p className="text-base sm:text-lg text-white/80 font-medium max-w-2xl leading-relaxed mb-4">
            A workforce training program that prepares you for entry-level HVAC technician roles —
            including EPA 608 certification, the federal credential required to handle regulated
            refrigerants.
          </p>
          <p className="text-sm text-white/60 max-w-2xl mb-5">
            Participants who complete the program are prepared to sit for EPA 608 certification and
            pursue roles such as HVAC Helper, Installer Assistant, or Entry-Level HVAC Technician.
          </p>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-4 text-white/70 text-sm">
              <span className="flex items-center gap-1.5">
                ...
                {totalHours > 0 ? `${totalHours}+ hrs` : `${remainingMinutes}m`}
              </span>
              <span className="flex items-center gap-1.5">
            ...
            {[
              {
                ...
                title: 'HVAC Helper → Technician',
                body: 'Targets entry-level roles: HVAC Helper, Installer Assistant, and Entry-Level HVAC Technician in residential and light commercial settings.',
              },
              {
                icon: BadgeCheck,
                color: 'text-amber-400',
                title: 'EPA 608 Certification Prep',
                body: 'Required by federal law to handle refrigerants. A standard requirement for HVAC technicians and widely expected by employers.',
              },
              {
                icon: DollarSign,
              ...
              <div key={title} className="flex items-start gap-3">
                <div
                  className={`w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0 mt-0.5`}
                >
          ...
          <div className="lg:col-span-2">
                ...
                {[
                  {
                    icon: BadgeCheck,
                    color: 'bg-amber-500',
                    label: 'EPA 608 Universal',
                    sub: 'Federal credential — required to handle regulated refrigerants',
                  },
                  {
                    icon: Shield,
                    color: 'bg-orange-600',
                    label: 'OSHA 10',
                    sub: 'DOL-issued Construction Safety card',
                  },
                  {
                    icon: Award,
                    color: 'bg-brand-blue-600',
                    label: 'CPR / First Aid',
                    sub: 'Certification issued through an industry-recognized training provider',
                  },
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 mb-6">
              ...
              <p className="text-slate-700 text-sm leading-relaxed">
                A workforce training program — not a survey course. You'll learn to diagnose,
                install, and service real HVAC systems, then sit for the EPA 608 Universal exam.
                Participants who complete the program are prepared to pursue entry-level HVAC
                technician roles. WIOA funding is available for eligible participants — no
                out-of-pocket cost for those who qualify.
              </p>
            ...
            <div className="mb-4">
              ...
              <p className="text-sm text-slate-500 mt-1">
                {modules.length} modules · {allLessons.length} lessons ·{' '}
                {totalHours > 0 ? `${totalHours}+ hours` : `${remainingMinutes}m`} of training
              </p>
            </div>
                  ...
                  <div className="mb-6 grid grid-cols-2 sm:grid-cols-5 gap-2">
                    {phases.map(({ label, name, range, color }) => {
                      const count = modules.filter(
                        (_: any, i: number) => i + 1 >= range[0] && i + 1 <= range[1],
                      ).length;
            ...
            {modules.length > 0 ? (
              ...
              <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
                {ungrouped.map((lesson: any, index: number) => {
                  ...
                  const prevDone =
                    index === 0 || progressMap.get(ungrouped[index - 1]?.id)?.completed;
                  const isLocked =
                    isPendingApproval ||
                    (!enrollment && index > 0) ||
                    (enrollment && !isCompleted && !prevDone);
                  const isCheckpoint =
              ...
              {isEnrolled ? (
                ...
                <div className="p-5">
                  <p className="text-xs text-slate-500 mb-3 text-center">
                    WIOA funding available for eligible participants
                  </p>
                  <Link
                    href={`/lms/courses/${courseId}/enroll`}
              ...
              <div className="space-y-3">
                {[
                  ...
                  },
                  {
                    icon: MapPin,
                    color: 'text-blue-400',
                    text: 'Strong demand across Indiana — residential, commercial, and industrial',
                  },
                  {
                    icon: DollarSign,
                    color: 'text-amber-400',
                    text: 'Entry-level roles start lower; progression into ~$45K–$75K+ range (BLS Indiana)',
                  },
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <ul className="space-y-2.5">
                  {(
                    ...
                    .filter(({ type }) => activityTypeSet.has(type))
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {program?.credential_type && (
                      <p className="text-xs text-slate-500 mt-0.5">{program.credential_type}</p>
                    )}
                  </div>
                </div>
              </div>
