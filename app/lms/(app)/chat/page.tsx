...
export default async function ChatPage() {
  ...
  const conversationList = Array.from(partnerIds).map((partnerId) => {
    ...
    const lastMsg = messages.find(
      (m: any) => m.sender_id === partnerId || m.recipient_id === partnerId,
    );
    return {
      partnerId,
      name: partner?.full_name || 'Unknown User',
      initials: (partner?.full_name || 'U')
        .split(' ')
        .map((n: string) => n[0])
        .join('')
        .toUpperCase()
        ...
        <div className="mb-6">
          <nav className="text-sm mb-4">
            <ol className="flex items-center space-x-2 text-slate-700">
              ...
              <li>/</li>
              <li className="text-slate-900 font-medium">Chat</li>
            </ol>
          </nav>
            ...
            <div className="divide-y">
              {conversationList.length > 0 ? (
                conversationList.map((conv, idx) => (
                  <div
                    key={conv.partnerId}
                    className={`p-4 hover:bg-white cursor-pointer ${idx === 0 ? 'bg-brand-blue-50' : ''}`}
                  >
              ...
              <>
                <div className="p-4 border-b">
                  <p className="font-semibold">
                    {conversationList[0]?.name || 'Select a conversation'}
                  </p>
                </div>
                  ...
                  <div className="space-y-4">
                    {messages
                      ...
                      .map((msg: any) => (
                        ...
                        >
                          <div
                            className={`rounded-lg p-3 max-w-xs ${msg.sender_id === user.id ? 'bg-brand-blue-600 text-white' : 'bg-white'}`}
                          >
