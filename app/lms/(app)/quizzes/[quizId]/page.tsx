...
export default async function QuizPage({ params }: Props) {
  ...
  const { data: quiz, error: quizError } = await supabase
    .from('quizzes')
    .select(
      'id, title, description, course_id, time_limit_minutes, passing_score, max_attempts, shuffle_questions, show_correct_answers, requires_proctoring, created_at',
    )
    .eq('id', quizId)
    .maybeSingle();
  ...
  const { data: questions } = await supabase
    .from('quiz_questions')
    .select(
      'id, question_text, question_type, points, order_index, quiz_answers (id, answer_text, is_correct, order_index)',
    )
    .eq('quiz_id', quizId)
    ...
    <div className="min-h-screen bg-white py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-8 mb-6">
          ...
          {quiz.max_attempts && (
            <div
              className={`rounded-xl p-4 mb-6 ${attemptsRemaining === 0 ? 'bg-red-50 border border-red-200' : 'bg-brand-blue-50 border border-brand-blue-200'}`}
            >
              <div className="flex items-center gap-2">
                <AlertCircle
                  className={`w-5 h-5 ${attemptsRemaining === 0 ? 'text-red-600' : 'text-brand-blue-600'}`}
                />
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center ${passed ? 'bg-brand-green-100' : 'bg-red-100'}`}
                      >
                        {passed ? (
                    ...
                    <div className="text-right">
                      <p
                        className={`text-2xl font-bold ${passed ? 'text-brand-green-600' : 'text-red-600'}`}
                      >
                        {attempt.score}%
                      </p>
