...
import { MessageSquare, Pin, Eye, MessageCircle, Clock, ChevronRight, Plus } from 'lucide-react';
...
export default async function ForumsPage() {
  ...
  const { data: topicCounts } = await supabase.from('forum_topics').select('category_id');
    ...
    <div className="min-h-screen bg-white">
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <Breadcrumbs
            items={[{ label: 'LMS', href: '/lms/dashboard' }, { label: 'Discussion Forums' }]}
          />
        </div>
      </div>
            ...
            {!categories || categories.length === 0 ? (
              <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center">
                ...
                <p className="text-slate-500 text-sm mt-1">
                  Categories will appear here once set up by an administrator.
                </p>
              </div>
            ) : (
              categories.map((cat) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${ICON_COLOR[cat.color ?? 'blue'] ?? ICON_COLOR.blue}`}
                    >
                  ...
                  {recentTopics.map((topic) => (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        {topic.is_pinned && (
                          <Pin className="w-3.5 h-3.5 text-brand-blue-500 flex-shrink-0 mt-0.5" />
                        )}
