        ...
        <div className="mb-8">
          <nav className="text-sm mb-4">
            <ol className="flex items-center space-x-2 text-slate-700">
              <li>
                <Link href="/lms/dashboard" className="hover:text-brand-blue-600">
                  LMS
                </Link>
              </li>
              <li>/</li>
              <li className="text-slate-900 font-medium">SCORM Content</li>
            </ol>
          </nav>
          <h1 className="text-3xl font-bold text-slate-900">SCORM Content</h1>
          <p className="text-slate-700 mt-2">
            Interactive learning modules with embedded progress tracking
          </p>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div
                      className={`w-10 h-10 rounded-lg flex items-center justify-center ${isCompleted ? 'bg-brand-green-100' : 'bg-brand-blue-100'}`}
                    >
                      <svg
                        className={`w-5 h-5 ${isCompleted ? 'text-brand-green-600' : 'text-brand-blue-600'}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                    ...
                    <span className="text-xs text-slate-500 bg-white px-2 py-1 rounded">
                      SCORM {pkg.scorm_version || '1.2'}
                    </span>
                    {isCompleted && (
                      <span className="text-xs text-brand-green-700 bg-brand-green-100 px-2 py-1 rounded font-medium">
                        Completed
                      </span>
                    )}
                  </div>
                  ...
                  <p className="text-sm text-slate-500 mb-3 line-clamp-2">
                    {pkg.description || 'Interactive SCORM module'}
                  </p>
                  {course && (
                    <p className="text-xs text-slate-400 mb-3">Course: {course.course_name}</p>
                  )}
                  {pkg.duration_minutes && (
                    <p className="text-xs text-slate-400 mb-3">{pkg.duration_minutes} min</p>
                  )}
                  {pct > 0 && !isCompleted && (
                    <div className="mb-3">
