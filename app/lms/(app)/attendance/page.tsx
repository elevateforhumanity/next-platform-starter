...
export default async function AttendancePage() {
  ...
  const totalHoursLogged =
    attendance?.reduce((sum: number, a: any) => sum + (a.hours_logged || 0), 0) || 0;
        ...
        <div className="mb-8">
          <nav className="text-sm mb-4">
            <ol className="flex items-center space-x-2 text-slate-700">
              ...
              <li>/</li>
              <li className="text-slate-900 font-medium">Attendance</li>
            </ol>
          </nav>
          ...
          <div className="divide-y">
            {attendance && attendance.length > 0 ? (
                  ...
                  <div key={record.id} className="p-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-700">
                        {new Date(record.date).toLocaleDateString()} &middot; {record.hours_logged}h
                        &middot; {record.type}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      {record.verified ? (
                        <span className="px-2 py-1 rounded-full text-xs bg-brand-green-100 text-brand-green-800">
                          Verified
                        </span>
                      ) : (
              ...
              <div className="p-8 text-center text-slate-700">
                No attendance records yet. Your instructor will log your hours.
              </div>
            )}
          </div>
        </div>
