...
const STEPS = [
  {
    num: '1',
    title: 'Apply',
    desc: 'Free application. Takes 5 minutes. No prior experience required.',
  },
  { num: '2', title: 'Train', desc: 'Complete lessons, pass checkpoints, and build real skills.' },
  {
    num: '3',
    title: 'Get Certified',
    desc: 'Earn your industry credential and connect with employers.',
  },
];
...
export default async function LmsPublicPage() {
  ...
  const { data: dbPrograms } = db
    ? await db
        .from('programs')
        .select(
          'id, title, slug, description, excerpt, image_url, duration_weeks, credential, credential_name, is_active, status',
        )
        .eq('is_active', true)
        .neq('status', 'archived')
        .order('title')
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-700 text-white font-bold px-8 py-4 rounded-xl text-base transition"
            >
              Enter Student Portal <ChevronRight className="w-4 h-4" />
            </Link>
            <Link
              href="/lms/programs"
              className="w-full sm:w-auto flex items-center justify-center gap-2 border-2 border-slate-200 hover:border-slate-400 text-slate-900 font-bold px-8 py-4 rounded-xl text-base transition"
            >
              Browse Programs
            </Link>
          </div>
          <p className="text-xs text-slate-400 mt-5">
            Free to apply · WIOA funding available for eligible Indiana residents
          </p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 text-base">
              Most programs are fully funded for Indiana residents.
            </p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {programs.map((p) => (
              <div
                key={p.slug}
                className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-md transition group"
              >
          ...
          <div className="text-center mt-8">
            <Link
              href="/lms/programs"
              className="inline-flex items-center gap-2 text-sm font-semibold text-slate-700 border border-slate-200 px-5 py-2.5 rounded-xl hover:bg-slate-50 transition"
            >
          ...
          <div className="grid sm:grid-cols-3 gap-8">
            {STEPS.map((s) => (
              <div key={s.num} className="flex flex-col items-center">
                <div className="w-12 h-12 rounded-full bg-slate-900 text-white text-xl font-extrabold flex items-center justify-center mb-4">
                  {s.num}
                </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-lg font-extrabold text-slate-900 mb-1">
              Funding Available for Indiana Residents
            </h3>
            <p className="text-slate-500 text-sm">
              WIOA, WorkOne, and employer-sponsored training may cover your full tuition.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 flex-shrink-0">
            <a
              ...
              className="text-sm font-semibold text-slate-700 border border-slate-200 bg-white px-5 py-2.5 rounded-xl hover:bg-slate-50 transition text-center"
            >
              Indiana Career Connect →
            </a>
            <Link
              href="/lms/programs"
              className="text-sm font-semibold text-white bg-slate-900 px-5 py-2.5 rounded-xl hover:bg-slate-700 transition text-center"
            >
      ...
      <section className="py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <p className="text-slate-600 text-base mb-2 text-center">
            Everything you need to study, practice, and succeed.
          </p>
          <StudentToolsStrip />
        </div>
      </section>
      ...
      <section className="py-16 bg-slate-900">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-slate-400 text-base mb-8">
            Create your account and enroll in minutes.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href={buildLoginRedirect('/lms/courses')}
              className="w-full sm:w-auto bg-brand-red-600 hover:bg-brand-red-700 text-white font-bold px-8 py-4 rounded-xl text-base transition"
            >
              Enter Student Portal
            </Link>
            <Link
              href="/lms/programs"
              className="w-full sm:w-auto border-2 border-slate-600 hover:border-slate-400 text-white font-bold px-8 py-4 rounded-xl text-base transition"
            >
              Browse Programs
            </Link>
          </div>
