...
const DOC_SLOTS = [
  ...
  {
    id: 'salon_license',
    label: 'Indiana Nail Salon License',
    description: 'Current Indiana nail salon license issued by IPLA.',
    required: true,
    accept: '.pdf,.jpg,.jpeg,.png',
  ...
  },
  {
    ...
    label: 'Supervising Nail Technician License',
    description: 'Indiana IPLA nail technician license for the designated apprentice supervisor.',
    required: true,
    accept: '.pdf,.jpg,.jpeg,.png',
...
export default function Nail TechnicianDocumentsPage() {
  ...
  async function handleFile(slotId: string, file: File) {
    ...
    try {
      ...
      fd.append('program', 'nail-technician-apprenticeship');
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-3xl mx-auto px-4 py-8">
        <InstitutionalHeader title="Required Documents" subtitle="Nail Technician Apprenticeship — Host Spa Partner" documentType="Document Upload" />
        ...
        <div className="mt-8 flex items-center justify-between">
          <Link href="/partners/nail-technician-apprenticeship/policy-acknowledgment" className="text-sm text-slate-500 hover:text-slate-700">← Back</Link>
          <Link href="/partners/nail-technician-apprenticeship/forms"
            className={`inline-flex items-center gap-2 font-bold px-6 py-3 rounded-xl transition-colors text-sm ${requiredDone ? 'bg-purple-600 hover:bg-purple-700 text-white' : 'bg-slate-200 text-slate-400 cursor-not-allowed pointer-events-none'}`}>
