...
const POLICIES = [
  ...
  },
  {
    ...
    summary: 'I understand the Indiana Professional Licensing Agency requirements for esthetician training.',
    details: [
      'I will ensure all supervised training meets Indiana IPLA standards',
      'I will maintain a licensed esthetician supervisor on-site during all apprentice training hours',
      'I understand that 1,500 hours are required for Indiana esthetician licensure',
...
export default function EstheticianPolicyAcknowledgmentPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await fetch('/api/partners/policy-acknowledgment', {
        ...
        body: JSON.stringify({ program: 'esthetician-apprenticeship', policies: Object.keys(acknowledged) }),
      });
      setSubmitted(true);
    } catch {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-8 max-w-md w-full text-center">
          ...
          <Link href="/partners/esthetician-apprenticeship/documents"
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-3xl mx-auto px-4 py-8">
        <InstitutionalHeader title="Policy Acknowledgment" subtitle="Esthetician Apprenticeship — Host Spa Partner" documentType="Policy Acknowledgment" />
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
                  ...
                  <ul className="mt-2 space-y-1">
                    {policy.details.map((d) => (
                      <li key={d} className="text-xs text-slate-500 flex items-start gap-1.5">
                        <BookOpen className="w-3 h-3 mt-0.5 shrink-0 text-pink-400" /> {d}
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="flex items-center justify-between pt-2">
            <Link href="/partners/esthetician-apprenticeship/handbook" className="text-sm text-slate-500 hover:text-slate-700">← Back to Handbook</Link>
