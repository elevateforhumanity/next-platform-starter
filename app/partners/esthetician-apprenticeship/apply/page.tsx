...
export default function EstheticianSpaApplyPage() {
  ...
  const [establishmentLicenseFile, setEstablishmentLicenseFile] = useState<File | null>(null);
  const [establishmentLicenseFileName, setEstablishmentLicenseFileName] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!establishmentLicenseFile) {
      setError('Please upload your Indiana establishment license.');
      return;
    }
    if (formData.hasGeneralLiability !== 'yes') {
      setError('General liability insurance is required for host sites.');
      return;
    }
    ...
    setError('');
    try {
      const shopLicenseFileData = await encodeDataUrlFile(establishmentLicenseFile);
      const insuranceFileData = insuranceFile ? await encodeDataUrlFile(insuranceFile) : undefined;
      const res = await fetch('/api/partners/esthetician-apprenticeship/apply', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...formData,
          shopLicenseFileData,
          shopLicenseFileName: establishmentLicenseFileName,
          insuranceFileData,
          insuranceFileName,
        }),
      });
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center py-12 px-4">
        <div className="max-w-lg w-full text-center">
          ...
          <p className="text-slate-600 mb-6">
            Your host spa application has been received. Our team will review it within 2–3 business
            days and contact you at <strong>{formData.contactEmail}</strong>.
          </p>
          <div className="bg-pink-50 border border-pink-200 rounded-xl p-4 mb-6 text-left">
            <p className="text-pink-900 font-bold text-sm mb-2">Next steps:</p>
            <ol className="space-y-1 text-pink-800 text-sm list-decimal list-inside">
              <li>Sign the MOU digitally</li>
              <li>Acknowledge the Partner Handbook</li>
              <li>Upload establishment license and supervisor license if not yet on file</li>
              <li>Receive apprentice placement</li>
            </ol>
          </div>
          <Link
            href="/login?redirect=/partners/esthetician-apprenticeship/sign-mou"
          ...
          <p className="text-sm text-slate-500 mt-4">
            ...
            <Link
              href="/signup?redirect=/partners/esthetician-apprenticeship/sign-mou"
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 mb-4">
                  Upload your establishment license now (required). Insurance COI is optional at
                  submit but required before final approval — same process as cosmetology and barber
                  host partners.
                </p>
                <HostShopComplianceUploads
                  licenseLabel="Indiana Establishment License"
                  licenseFileName={establishmentLicenseFileName}
                  onLicenseChange={(f, name) => {
                    setEstablishmentLicenseFile(f);
                    setEstablishmentLicenseFileName(name);
                  }}
              ...
              <div className="space-y-3 bg-slate-50 rounded-xl p-4 border border-slate-100">
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <span className="text-xs text-slate-700">
                    ...
                    for the Esthetician Apprenticeship Program and understand my responsibilities as a
                    host spa or salon.
                  </span>
                </label>
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <span className="text-xs text-slate-700">
                    ...
                    the esthetician apprenticeship program. I confirm the information provided is
