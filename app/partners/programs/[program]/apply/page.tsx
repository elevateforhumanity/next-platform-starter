...
const REGULATED_SLUGS: Record<string, string> = {
  'barbershop-apprenticeship': '/partners/barber-host-shop/apply',
      ...
      <div className="max-w-3xl mx-auto px-4 pb-16">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">
            {config.shortName} Partner Application
          </h1>
          <p className="text-black">
            Become a partner {config.siteLabel} for the {config.name} program.
          </p>
        </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-8">
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="businessLegalName" className={labelClass}>
                ...
                <div>
                  <label htmlFor="dba" className={labelClass}>
            ...
            {step === 2 && (
              <div className="space-y-4">
                <h2 className="text-xl font-bold text-slate-900 mb-4">
                  {config.siteLabel.charAt(0).toUpperCase() + config.siteLabel.slice(1)} Location
                </h2>
                <div>
            ...
            {step === 3 && (
              <div className="space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="compensationModel" className={labelClass}>
                    ...
                    >
                      <option value="">Select...</option>
                      {config.compensationModels.map((opt) => (
                        <option key={opt.value} value={opt.value}>
                          {opt.label}
                        </option>
                      ))}
                  ...
                  <div>
                    <label htmlFor="positionsAvailable" className={labelClass}>
            ...
            {step === 4 && (
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg p-4 text-sm text-black space-y-2">
                  <p>
                    <strong>Business:</strong> {formData.businessLegalName}{' '}
                  ...
                  <p>
                    <strong>Location:</strong> {formData.addressLine1}, {formData.city},{' '}
                    {formData.state} {formData.zip}
                  </p>
                  <p>
                    <strong>Program:</strong> {config.name}
                  </p>
                  <p>
                    <strong>Positions:</strong> {formData.positionsAvailable}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {config.siteVisitRequired && (
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        name="agreeToSiteVisit"
                        checked={formData.agreeToSiteVisit}
                        onChange={handleChange}
                        required
                        className="mt-1 w-4 h-4 text-orange-500"
                      />
                      <span className="text-sm text-slate-900">
                        I agree to complete a video site visit (via Zoom) before final approval.
                      </span>
                    </label>
                  )}
                  {config.workersCompRequired && (
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        name="agreeToWorkersComp"
                        checked={formData.agreeToWorkersComp}
                        onChange={handleChange}
                        required
                        className="mt-1 w-4 h-4 text-orange-500"
                      />
                      <span className="text-sm text-slate-900">
                        I acknowledge that workers&apos; compensation insurance is required for{' '}
                        {config.traineeLabelPlural}.
                      </span>
                    </label>
                  )}
                </div>
            ...
            <div className="flex justify-between mt-8">
              ...
              {step < totalSteps ? (
                <button
                  type="button"
                  onClick={() => setStep((s) => s + 1)}
                  className="px-8 py-3 bg-orange-500 text-white rounded-lg font-bold hover:bg-orange-600 transition"
                >
                  Continue
                </button>
              ) : (
                <button
                  ...
                  className="px-8 py-3 bg-orange-500 text-white rounded-lg font-bold hover:bg-orange-600 transition disabled:opacity-50"
                >
