...
const C = {
  blue: {
    ...
    label: 'text-brand-blue-600',
    btn: 'bg-brand-blue-600 hover:bg-brand-blue-700 text-white',
    outline: 'border-2 border-brand-blue-300 text-brand-blue-700 hover:bg-brand-blue-50',
  },
  green: {
    border: 'border-brand-green-200',
    badge: 'bg-brand-green-100 text-brand-green-700',
    label: 'text-brand-green-600',
    btn: 'bg-brand-green-600 hover:bg-brand-green-700 text-white',
    outline: 'border-2 border-brand-green-300 text-brand-green-700 hover:bg-brand-green-50',
  },
  red: {
    border: 'border-brand-red-200',
    badge: 'bg-brand-red-100 text-brand-red-700',
    label: 'text-brand-red-600',
    btn: 'bg-brand-red-600 hover:bg-brand-red-700 text-white',
    outline: 'border-2 border-brand-red-300 text-brand-red-700 hover:bg-brand-red-50',
  },
} as const;
      ...
      <section className="relative w-full">
        ...
        <div className="bg-white py-10 border-t">
          <div className="max-w-5xl mx-auto px-4 text-center">
            <p className="font-semibold text-sm mb-2 uppercase tracking-wide text-brand-blue-600">
            ...
            <p className="text-lg text-black max-w-3xl mx-auto">
              We work with workforce agencies, employers, barbershops, training providers, and
              reentry organizations. Find your type below and apply in minutes.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="grid sm:grid-cols-3 gap-6 text-center">
            {[
              {
                step: '1',
                heading: 'Find your type',
                body: 'Scroll down and identify which partnership category fits your organization.',
              },
              {
                step: '2',
                heading: 'Read what is required',
                body: 'Each section explains who qualifies, what you get, and what we need from you.',
              },
              {
                step: '3',
        ...
        <div className="max-w-5xl mx-auto px-4 flex flex-col gap-8">
          {PARTNER_TYPES.map((type) => {
            ...
            return (
              <div
                key={type.title}
                className={`bg-white rounded-2xl border-2 ${c.border} overflow-hidden shadow-sm`}
              >
                <div className="grid md:grid-cols-5">
                  ...
                  <div className="md:col-span-3 p-6 flex flex-col gap-4">
                    <div
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold self-start ${c.badge}`}
                    >
                      <type.icon className="w-3.5 h-3.5" />
                      {type.title}
                    </div>
                    <div>
                      <p className={`text-xs font-bold uppercase tracking-widest mb-1 ${c.label}`}>
                        Who this is for
                      </p>
                    ...
                    <div>
                      <p className={`text-xs font-bold uppercase tracking-widest mb-2 ${c.label}`}>
                        How to get started
                      </p>
                      <ol className="flex flex-col gap-1.5">
                        {type.steps.map((step, i) => (
                          <li key={i} className="flex items-start gap-2 text-sm text-slate-700">
                            <span
                              className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5 ${c.badge}`}
                            >
                              {i + 1}
                            </span>
                            {step}
                    ...
                    <div className="flex flex-wrap gap-3 pt-1 border-t border-slate-100">
                      ...
                      <Link
                        href={type.cta.href}
                        className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-lg font-semibold text-sm transition-colors ${c.outline}`}
                      >
      ...
      <section className="py-12 border-t border-slate-100">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-black mb-6">
            Call us and we will figure it out together. Most partnerships are set up within a week.
          </p>
