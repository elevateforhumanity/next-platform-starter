...
export const metadata: Metadata = {
  ...
  description:
    'Printable master competency scoring rubric for barber apprenticeship. 30 competencies, 0–5 scale, evaluator signature lines.',
  alternates: {
    canonical:
      'https://www.elevateforhumanity.org/compliance/competency-verification/barber/scoring-sheet',
  },
};
      ...
      <div className="hidden print:block px-6 pt-6 pb-3 border-b-2 border-gray-900">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right text-[10px] text-slate-700">
            ...
            <p>
              {BARBER_STATS.totalCompetencies} competencies | {BARBER_STATS.sections} sections
            </p>
          </div>
        </div>
      ...
      <div className="print:hidden py-8 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">
            Barber Apprenticeship Scoring Sheet
          </h1>
          <p className="text-slate-700 mb-4">
            Printable master scoring rubric. Print this page or save as PDF to use during competency
            evaluations.
          </p>
          <PrintButton />
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-4 print:px-6 print:py-2">
        <div className="border rounded-lg p-3 print:p-2 print:border-gray-400">
          ...
          <div className="grid grid-cols-6 gap-1 text-xs print:text-[9px] text-center">
            <div className="border rounded py-1 bg-white print:bg-white">
              <strong>0</strong>
              <br />
              Not Demonstrated
            </div>
          ...
          <p className="text-xs text-slate-700 mt-2 print:text-[9px] print:mt-1">
            Minimum passing standard: Score of 3 (Competent) in all core competencies. Sanitation
            failures require automatic remediation.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-8 print:px-6 print:pb-4">
          ...
          <div key={section.section} className="mb-6 print:mb-4 print:break-inside-avoid">
            <div className="bg-brand-blue-700 text-white px-4 py-2 rounded-t print:rounded-none">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs text-slate-600 print:text-[9px]">
                  {section.items.length} competencies
                </span>
              </div>
            </div>
            ...
            <table className="w-full border-collapse text-sm print:text-[10px]">
              <thead>
                <tr className="bg-white print:bg-white">
                  ...
                  <th className="text-center p-2 border font-semibold w-16 print:p-1">
                    Score (0–5)
                  </th>
              ...
              <tbody>
                {section.items.map((item) => (
                  <tr key={item.id} className="print:break-inside-avoid">
                    <td className="p-2 border text-xs font-mono print:p-1 print:text-[9px]">
                      {item.id}
                    </td>
                    <td className="p-2 border print:p-1">{item.competency}</td>
                    <td className="p-2 border text-center print:p-1">
                      {item.rtiHours > 0 ? `${item.rtiHours}h` : '—'}
                    </td>
                    <td className="p-2 border text-center print:p-1">
                      {item.ojtHours > 0 ? `${item.ojtHours}h` : '—'}
                    </td>
                    <td className="p-2 border text-center print:p-1">
        ...
        <div className="border-t-2 border-gray-900 pt-6 mt-8 print:pt-4 print:mt-4">
          ...
          <div className="grid grid-cols-1 gap-6 print:gap-3">
            {[
              {
                role: 'RTI Instructor (Credential Partner)',
                scope: 'Sections 1–3, 5 — Technical & theoretical competencies',
              },
              {
                role: 'Employer Barbershop Supervisor',
                scope: 'Sections 4–6 — OJT performance & workplace readiness',
              },
              {
                role: 'Program Holder (RTI Coordinator)',
                scope: 'Rubric completion verification & LMS documentation',
              },
              {
                role: 'Elevate for Humanity (Sponsor)',
                scope: 'Final completion validation & apprenticeship documentation',
              },
            ].map((signer, i) => (
        ...
        <div className="border-2 border-gray-900 rounded-lg p-4 mt-6 print:p-3 print:mt-4">
          <h3 className="font-bold text-sm mb-2 print:text-[11px]">
