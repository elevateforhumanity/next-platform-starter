...
export const metadata: Metadata = {
  ...
  description:
    'Choose your license for the Elevate Workforce Operating System. Managed platform or enterprise source-use.',
  alternates: {
      ...
      <div className="bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-3">
          ...
          <p className="text-sm text-slate-600 mt-1">
            ...
            .{' '}
            <a href="/platform/overview" className="hover:underline">
              How it works →
            </a>
          </p>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="mt-3 text-sm text-slate-600">
                Not sure yet?{' '}
                <Link href="/store/trial" className="text-brand-red-600 hover:underline">
                  Start a free 14-day trial
                </Link>{' '}
                or{' '}
                <Link href="/store/demos" className="text-brand-red-600 hover:underline">
                  view the demo
                </Link>{' '}
                first.
              </p>
            ...
            {licenses.map((lic) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-4">
                  <lic.icon
                    className={`w-8 h-8 ${lic.popular ? 'text-brand-red-400' : 'text-brand-red-600'}`}
                  />
                  <h2
                    className={`text-2xl font-bold ${lic.popular ? 'text-slate-900' : 'text-slate-900'}`}
                  >
                    {lic.name}
                  </h2>
                </div>
                ...
                <p
                  className={`text-lg font-medium mb-2 ${lic.popular ? 'text-slate-900/90' : 'text-slate-800'}`}
                >
                  {lic.tagline}
                </p>

                <p
                  className={`text-sm mb-4 ${lic.popular ? 'text-slate-900/70' : 'text-slate-600'}`}
                >
                  {lic.who}
                </p>

                <p
                  className={`text-3xl font-black mb-6 ${lic.popular ? 'text-slate-900' : 'text-slate-900'}`}
                >
                  {lic.price}
                </p>
                ...
                <ul className="space-y-3 mb-8 flex-grow">
                  {lic.included.map((item) => (
                    <li key={item} className="flex items-start gap-2 text-sm">
                      ...
                      <span className={lic.popular ? 'text-slate-900/90' : 'text-slate-800'}>
                        {item}
                      </span>
                    </li>
                  ))}
            ...
            {[
              {
                ...
                a: 'Yes. Start a free 14-day trial with no credit card required, or take a guided demo tour to see each role in action.',
              },
              {
                q: 'What happens after I purchase a license?',
                a: 'Managed Platform: We set up your branded instance within 5 business days. Source-Use: Legal review and repository access within 2 weeks after agreement execution.',
              },
              {
                q: 'Can I cancel my managed platform subscription?',
                a: 'Yes. Monthly subscriptions can be cancelled with 30 days notice. Annual contracts are non-refundable but will not auto-renew unless requested.',
              },
              {
                q: 'Do I own the software?',
                a: 'No. All licenses grant access to use the platform. Ownership of software, IP, and infrastructure remains with Elevate for Humanity.',
              },
              {
                q: 'Is the platform FERPA and WIOA compliant?',
                a: 'Yes. The platform is built for workforce development and includes FERPA-compliant data handling, WIOA reporting templates, and WCAG 2.1 AA accessibility.',
              },
              {
                q: 'Can I use my own domain and branding?',
                a: 'Yes. All managed platform licenses include custom domain setup and full branding (logo, colors, email templates).',
              },
            ].map((faq) => (
              <details key={faq.q} className="group border border-slate-200 rounded-lg">
                <summary className="flex items-center justify-between p-5 cursor-pointer font-semibold text-slate-900 hover:bg-white">
                  {faq.q}
                  <svg
                    className="w-5 h-5 text-slate-600 group-open:rotate-180 transition-transform flex-shrink-0 ml-4"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
      ...
      <section className="py-12 border-t border-slate-200">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-slate-800 mb-6">
            Most organizations start with Managed Platform. If you have a dedicated engineering team
            and need internal deployment, Enterprise Source-Use may be right.
          </p>
          <Link
          ...
          <p className="mt-4 text-sm text-slate-600">
            <Link href="/platform" className="text-slate-600 hover:underline">
              Learn how the platform works
            </Link>
            {' · '}
            <Link href="/contact?topic=licensing" className="text-slate-600 hover:underline">
              Licensing questions
            </Link>
          </p>
        </div>
