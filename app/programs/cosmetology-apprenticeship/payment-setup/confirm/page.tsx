...
function ConfirmInner() {
  ...
  useEffect(() => {
    ...
    fetch('/api/cosmetology/activate-subscription', { method: 'POST' })
      .then((r) => r.json())
      .then((data) => {
        if (data.error) {
          setStatus('error');
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center px-4">
      <div className="text-center max-w-md">
        ...
        <button
          onClick={() => router.push('/programs/cosmetology-apprenticeship/payment-setup')}
...
export default function CosmoPaymentSetupConfirmPage() {
  return <ConfirmInner />;
}
