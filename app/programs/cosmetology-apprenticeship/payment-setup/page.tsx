...
function PaymentSetupForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: confirmError } = await stripe.confirmSetup({
      elements,
      confirmParams: {
        return_url: `${window.location.origin}/programs/cosmetology-apprenticeship/payment-setup/confirm`,
      },
    ...
    // Setup succeeded — activate subscription then go to orientation
    try {
      const res = await fetch('/api/cosmetology/activate-subscription', { method: 'POST' });
      if (!res.ok) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <p className="text-center text-xs text-slate-500">
        Down payment of{' '}
        <strong className="text-slate-300">${deposit.toLocaleString()}</strong> charged today. Then{' '}
        <strong className="text-slate-300">${(weeklyAmount / 100).toFixed(2)}/week</strong> for{' '}
        {PAYMENT_TERM_WEEKS} weeks.
      </p>
    </form>
  );
}
...
const MIN_DEPOSIT = MIN_SETUP_FEE_CENTS / 100; // $250
