...
export default async function BarberOrientationPage() {
  ...
  let payment: BarberPaymentSummary = {
    ...
    weeklyPaymentCents: Math.round(
      ((BARBER_PRICING.fullPrice - BARBER_PRICING.minDownPayment) /
  ...
  if (user) {
    ...
    const { data: sub } = await db
      ...
      .select(
        'amount_paid_at_checkout, remaining_balance, weekly_payment_cents, weeks_remaining, fully_paid',
      )
    ...
    if (sub) {
      payment = {
        ...
        remainingBalance:
          ...
          BARBER_PRICING.fullPrice - (sub.amount_paid_at_checkout ?? BARBER_PRICING.minDownPayment),
