...
export const metadata: Metadata = { robots: { index: false, follow: false }, title: 'Program Analytics | Admin' };
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="bg-white border-b border-slate-200 px-6 py-5">
        ...
        <p className="text-sm text-slate-500 mt-1">Enrollment, completion, and credential outcomes by program</p>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Programs',    value: totalPrograms,    icon: BookOpen,    color: 'text-brand-blue-600', bg: 'bg-brand-blue-50' },
            { label: 'Active Programs',   value: activePrograms,   icon: TrendingUp,  color: 'text-green-600',      bg: 'bg-green-50' },
            { label: 'Total Enrollments', value: totalEnrollments, icon: Users,       color: 'text-purple-600',     bg: 'bg-purple-50' },
            { label: 'Certificates',      value: totalCerts,       icon: Award,       color: 'text-amber-600',      bg: 'bg-amber-50' },
          ].map((s) => {
            const Icon = s.icon;
            return (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          ...
          <p className="text-xs text-slate-400 mt-2">{completedEnroll} of {totalEnrollments} enrollments completed</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <Link href="/admin/programs" className="text-xs font-semibold text-brand-blue-600 hover:underline flex items-center gap-1">Manage <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <table className="w-full text-sm">
            <thead><tr className="border-b border-slate-100 bg-slate-50">
              {['Program','Category','Enrollments','Duration','Status'].map(h => <th key={h} className="text-left py-3 px-5 text-[10px] font-bold uppercase tracking-widest text-slate-400">{h}</th>)}
            </tr></thead>
            <tbody className="divide-y divide-slate-50">
              {programs
                .sort((a: any, b: any) => (enrollByProgram[b.id] ?? 0) - (enrollByProgram[a.id] ?? 0))
                .map((p: any) => (
                <tr key={p.id} className="hover:bg-slate-50">
                  <td className="py-3.5 px-5">
                    <Link href={`/admin/programs/${p.id}`} className="font-semibold text-slate-900 hover:text-brand-blue-700">{p.title ?? '—'}</Link>
                    {p.credential_type && <p className="text-xs text-slate-400">{p.credential_type}</p>}
                  </td>
                  <td className="py-3.5 px-5 text-slate-500 text-xs capitalize">{p.category ?? '—'}</td>
                  <td className="py-3.5 px-5 font-semibold text-slate-900 tabular-nums">{enrollByProgram[p.id] ?? 0}</td>
                  <td className="py-3.5 px-5 text-slate-500 text-xs">{p.estimated_weeks ? `${p.estimated_weeks}w` : '—'}</td>
                  <td className="py-3.5 px-5">
                    <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-semibold ${p.is_active ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-500'}`}>
