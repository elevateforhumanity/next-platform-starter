...
import { ArrowLeft, MapPin, Clock, CheckCircle, Loader2, Flower2, BookOpen, TrendingUp } from 'lucide-react';
...
export default function EstheticianCheckinPage() {
  ...
  const handleCheckin = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          const res = await fetch('/api/pwa/esthetician/checkin', {
            method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-slate-900 pb-20">
      <header className="bg-rose-700 px-4 pt-12 pb-6 safe-area-inset-top">
        ...
        <p className="text-rose-200 text-sm">Verify your location to record attendance at your training site.</p>
      </header>
      ...
      <main className="px-4 py-8 flex flex-col items-center gap-6">
        ...
        {status !== 'success' && (
          <button
            ...
            className="w-full py-4 bg-rose-600 text-white font-bold text-lg rounded-xl disabled:opacity-50 hover:bg-rose-700 transition"
          >
