...
const DEFAULT_STEPS: OnboardingStep[] = [
  ...
  { id: 'documents',     title: 'Upload Your ID',             description: 'Upload a government-issued photo ID to verify your identity.',                       icon: 'file',     completed: false, actionUrl: '/programs/nail-technician-apprenticeship/documents', actionLabel: 'Upload Documents' },
  { id: 'salon',         title: 'Confirm Your Training Site', description: 'Your nail salon must be approved before you can log hours.',                         icon: 'building', completed: false, actionUrl: '/pwa/nail-tech',                                  actionLabel: 'View Status' },
  { id: 'orientation',   title: 'Complete Orientation',       description: 'Review program requirements, your rights, and what to expect.',                      icon: 'book',     completed: false, actionUrl: '/programs/nail-technician-apprenticeship/orientation', actionLabel: 'Start Orientation' },
  { id: 'hours',         title: 'Log Your First Hours',       description: 'Once your site is approved, start logging your weekly training hours.',              icon: 'clock',    completed: false, actionUrl: '/pwa/nail-tech/log-hours',                        actionLabel: 'Log Hours' },
...
export default function NailTechOnboardingPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!session) { window.location.href = '/login?redirect=/pwa/nail-tech/onboarding'; return; }
      ...
      const { data: orientation } = await supabase
        ...
        .eq('user_id', session.user.id).eq('program_slug', 'nail-technician-apprenticeship').limit(1);
      ...
      <header className="bg-pink-700 px-4 pt-12 pb-6 safe-area-inset-top">
        ...
        <div className="bg-white/10 rounded-xl p-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-pink-200 text-sm">{completedCount} of {steps.length} steps complete</span>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {!step.completed && !locked && step.actionUrl && (
                      <Link href={step.actionUrl} className="inline-flex items-center gap-1 mt-2 text-pink-400 text-sm font-medium hover:text-pink-300">
                    ...
                    {!step.completed && !locked && !step.actionUrl && step.actionLabel === 'Enable' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 mt-2 text-pink-400 text-sm font-medium hover:text-pink-300"
                      >
