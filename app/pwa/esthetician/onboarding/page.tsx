...
const DEFAULT_STEPS: OnboardingStep[] = [
  ...
  { id: 'documents',     title: 'Upload Your ID',             description: 'Upload a government-issued photo ID to verify your identity.',                     icon: 'file',     completed: false, actionUrl: '/programs/esthetician/documents',       actionLabel: 'Upload Documents' },
  { id: 'salon',         title: 'Confirm Your Training Site', description: 'Your spa or salon must be approved before you can log hours.',                     icon: 'building', completed: false, actionUrl: '/pwa/esthetician',                      actionLabel: 'View Status' },
  { id: 'orientation',   title: 'Complete Orientation',       description: 'Review program requirements, your rights, and what to expect.',                    icon: 'book',     completed: false, actionUrl: '/programs/esthetician/orientation',     actionLabel: 'Start Orientation' },
  { id: 'hours',         title: 'Log Your First Hours',       description: 'Once your site is approved, start logging your weekly training hours.',            icon: 'clock',    completed: false, actionUrl: '/pwa/esthetician/log-hours',            actionLabel: 'Log Hours' },
...
export default function EstheticianOnboardingPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!session) { window.location.href = '/login?redirect=/pwa/esthetician/onboarding'; return; }

      setSteps(prev => prev.map(s => s.id === 'account' ? { ...s, completed: true } : s));

      const { data: docs } = await supabase
        .from('user_documents').select('id').eq('user_id', session.user.id).limit(1);
      if (docs?.length) setSteps(prev => prev.map(s => s.id === 'documents' ? { ...s, completed: true } : s));

      const { data: orientation } = await supabase
        .from('orientation_completions').select('id')
        .eq('user_id', session.user.id).eq('program_slug', 'esthetician-apprenticeship').limit(1);
      if (orientation?.length) setSteps(prev => prev.map(s => s.id === 'orientation' ? { ...s, completed: true } : s));

      const { data: hours } = await supabase
        .from('apprentice_hours').select('id').eq('user_id', session.user.id).limit(1);
      if (hours?.length) setSteps(prev => prev.map(s => s.id === 'hours' ? { ...s, completed: true } : s));
      ...
      <header className="bg-rose-700 px-4 pt-12 pb-6 safe-area-inset-top">
        ...
        <div className="bg-white/10 rounded-xl p-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-rose-200 text-sm">{completedCount} of {steps.length} steps complete</span>
            <span className="text-white font-bold">{Math.round((completedCount / steps.length) * 100)}%</span>
          </div>
          <div className="h-2 bg-rose-900 rounded-full overflow-hidden">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {!step.completed && !locked && step.actionUrl && (
                      <Link href={step.actionUrl} className="inline-flex items-center gap-1 mt-2 text-rose-400 text-sm font-medium hover:text-rose-300">
                    ...
                    {!step.completed && !locked && !step.actionUrl && step.actionLabel === 'Enable' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 mt-2 text-rose-400 text-sm font-medium hover:text-rose-300"
                      >
