...
export default async function ProgramHolderDashboardPage() {
  ...
  const [programsRes, enrollmentsRes, completedRes, certsRes, recentLearnersRes, paymentsRes] =
    await Promise.all([
      programIds.length > 0
        ? db
            .from('programs')
      ...
      programIds.length > 0
        ? db
            .from('program_enrollments')
            .select(
              'id, enrolled_at, status, progress_percent, program_id, user_id, programs ( name, title )',
            )
            .in('program_id', programIds)
  ...
  const learnerUserIds = [
    ...new Set((recentLearnersRes.data ?? []).map((e: any) => e.user_id).filter(Boolean)),
  ];
          ...
          <div className="max-w-7xl mx-auto w-full px-6 pb-8 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="text-slate-300 text-sm mt-2">
                Manage your programs, learners, and compliance from one place.
              </p>
            </div>
            <Link
              href="/program-holder/settings"
              className="flex-shrink-0 p-3 bg-white/10 hover:bg-white/20 rounded-xl text-white transition"
            >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-amber-900 mb-2">
                  Complete your onboarding — {pendingSteps.length} step
                  {pendingSteps.length > 1 ? 's' : ''} remaining
                </p>
          ...
          {[
            {
              label: 'Active Learners',
              value: totalActive.toLocaleString(),
              img: '/images/pages/admin-students-hero.jpg',
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              ...
              {programs.length > 0 ? (
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {programs.map((p) => (
                      <tr key={p.id}>
                        <td className="py-3">
                          <Link
                            href={`/program-holder/programs/${p.id}`}
                            className="font-medium text-slate-800 hover:text-brand-blue-600"
                          >
                            {p.name}
                          </Link>
                        </td>
                        <td className="py-3 text-center text-slate-600">
                          {(enrollmentsByProgram[p.id] ?? 0).toLocaleString()}
                        </td>
                        <td className="py-3 text-center">
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${p.isActive ? 'bg-brand-green-100 text-brand-green-700' : 'bg-slate-100 text-slate-500'}`}
                          >
                ...
                <div className="text-center py-10">
                  ...
                  <Link
                    href="/program-holder/programs"
                    className="inline-block mt-3 text-sm text-brand-blue-600 hover:underline"
                  >
                    Add a program →
                  </Link>
                </div>
                    ...
                    <div key={l.id} className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="w-8 h-8 bg-brand-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
                          <span className="text-brand-blue-600 text-xs font-bold">
          ...
          <div className="space-y-4">
            ...
            {[
              ...
              {
                label: 'View Reports',
                sub: 'Compliance & performance',
                href: '/program-holder/reports',
                img: '/images/pages/admin-analytics-programs-hero.jpg',
              },
              {
                label: 'Documents',
              ...
              <Link
                key={action.href}
                href={action.href}
                className="group relative flex items-center gap-4 bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow"
              >
            ...
            <Link
              href="/program-holder/programs"
              className="group relative flex items-center gap-4 bg-brand-blue-600 rounded-2xl overflow-hidden shadow-sm hover:bg-brand-blue-700 transition-colors"
            >
              <div className="relative w-20 h-20 flex-shrink-0">
                <Image
                ...
                <div className="flex gap-2">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-amber-800">
                      {programs.filter((p) => !p.isActive).length} inactive program
                      {programs.filter((p) => !p.isActive).length > 1 ? 's' : ''}
                    </p>
                    <p className="text-xs text-amber-700 mt-0.5">Not visible to learners.</p>
                    <Link
                      href="/program-holder/programs"
                      className="text-xs text-amber-800 font-semibold hover:underline mt-1 inline-block"
                    >
                      Review →
                    </Link>
                  </div>
