...
export default async function SchedulePage() {
      ...
      <div className="min-h-screen bg-white py-8">
        <div className="max-w-4xl mx-auto px-4">
          <nav className="flex items-center gap-2 text-sm text-slate-700 mb-6">
            ...
            <Link href="/student-support" className="hover:text-brand-orange-600">
              Student Support
            </Link>
            <ChevronRight className="w-4 h-4" />
          ...
          <p className="text-slate-700 mb-8">
            Book a meeting with an advisor or counselor. Available Monday–Friday, 9 AM – 5 PM EST.
          </p>
          <div className="bg-white rounded-xl border p-8 text-center">
            <h2 className="text-xl font-bold mb-4">Sign in to schedule your appointment</h2>
            <p className="text-slate-700 mb-6">
              Create a free account or sign in to book a time with one of our advisors. Pick a time
              that works for you.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/login?redirect=/student-support/schedule"
                className="bg-brand-blue-600 text-white font-bold px-8 py-3 rounded-lg hover:bg-brand-blue-700 transition"
              >
                Sign In
              </Link>
              <Link
                href="/signup"
                className="border-2 border-brand-blue-600 text-brand-blue-600 font-bold px-8 py-3 rounded-lg hover:bg-brand-blue-50 transition"
              >
                Create Free Account
              </Link>
            </div>
            <p className="text-sm text-slate-700 mt-6">
              ...
              or{' '}
              <Link href="/faq" className="text-brand-blue-600 hover:underline">
                check our FAQ
              </Link>
            </p>
          </div>
  ...
  const { data: existingAppointments } = await supabase
    .from('appointments')
    .select(
      'id, appointment_date, appointment_time, status, advisor:profiles!appointments_advisor_id_fkey(full_name)',
    )
