...
export function CourseBuilderPageClient() {
      ...
      .then((d) => {
        if (d?.profiles)
          setComplianceProfiles(
            d.profiles.map((p: any) => ({ key: p.key, label: p.label ?? p.key })),
          );
      })
      .catch(() => {});
  ...
  const upMod = (i: number, p: Partial<BuilderModule>) =>
    setProgram((prev) => ({
      ...prev,
      modules: prev.modules.map((m, mi) => (mi === i ? { ...m, ...p } : m)),
    }));
  const upLes = (mi: number, li: number, p: Partial<BuilderLesson>) =>
    setProgram((prev) => ({
      ...
      modules: prev.modules.map((m, mIdx) =>
        mIdx !== mi
          ? m
          : { ...m, lessons: m.lessons.map((l, lIdx) => (lIdx !== li ? l : { ...l, ...p })) },
      ),
    }));
    ...
    <div className="p-6 space-y-4 text-sm">
      ...
      <div className="border-2 border-indigo-200 rounded-2xl p-5 bg-indigo-50 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select
              ...
              value={selBlueprint}
              onChange={(e) => setSelBlueprint(e.target.value)}
              disabled={generating}
            >
              <option value="">— Select blueprint —</option>
              {blueprints.map((b) => (
                <option key={b.id} value={b.id}>
                  {b.title} ({b.state}) — {b.modules}M / {b.lessons}L
                </option>
              ))}
            </select>
          </div>
        ...
        <button
          ...
          disabled={!selBlueprint || !selProgram || generating}
        ...
        {genResult && (
          <div
            className={`rounded-xl p-4 space-y-2 ${genResult.ok ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}
          >
            {genResult.ok ? (
              <>
                ...
                <div className="grid grid-cols-3 gap-2 text-xs text-green-700">
                  ...
                  <div>
                    <b>{genResult.lessonsInserted}</b> lessons
                  </div>
                  <div>
                    <b>{genResult.assessmentsGenerated}</b> quiz questions
                  </div>
                  <div>
                    <b>{genResult.videosQueued}</b> videos queued
                  </div>
                  <div>
                    <b>{genResult.contentFailures?.length ?? 0}</b> content failures
                  </div>
                  <div>
                ...
                {(genResult.contentFailures?.length ?? 0) > 0 && (
                  <details className="text-xs text-amber-700">
                    <summary className="cursor-pointer font-semibold">
                      ⚠️ {genResult.contentFailures!.length} lessons need review
                    </summary>
                    <ul className="mt-1 pl-3 space-y-0.5">
                      {genResult.contentFailures!.map((f) => (
                        <li key={f.slug}>• {f.slug}</li>
                      ))}
                    </ul>
                  </details>
                )}
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-3 border rounded-2xl p-4 space-y-2">
          ...
          <button className={`${btn} w-full`} onClick={addModule}>
            + Add Module
          </button>
        ...
        <div
          className="col-span-4 border rounded-2xl p-4 space-y-2 overflow-y-auto"
        ...
        >
          ...
          <input
            ...
            placeholder="Title"
            value={program.title}
            onChange={(e) => upProg({ title: e.target.value })}
          />
          <input
            className={inp}
            placeholder="Slug"
            value={program.slug}
            onChange={(e) => upProg({ slug: e.target.value })}
          />
          <input
            className={inp}
            type="number"
            placeholder="Min hours"
            value={program.minimumHours}
            onChange={(e) => upProg({ minimumHours: Number(e.target.value) })}
          />
          <select
            className={sel}
            value={program.credentialTarget}
            onChange={(e) => upProg({ credentialTarget: e.target.value as any })}
          >
            {['INTERNAL', 'STATE_BOARD', 'IC&RC', 'NAADAC', 'DOL_APPRENTICESHIP', 'CUSTOM'].map(
              (v) => (
          ...
          <select
            className={sel}
            value={program.regulatory.complianceProfileKey}
            onChange={(e) => upReg({ complianceProfileKey: e.target.value })}
          >
          ...
          <input
            ...
            value={program.regulatory.governingBody ?? ''}
            onChange={(e) => upReg({ governingBody: e.target.value || null })}
          />
          <input
            className={inp}
            type="number"
            placeholder="Retention policy days"
            value={program.regulatory.retentionPolicyDays ?? ''}
            onChange={(e) =>
              upReg({ retentionPolicyDays: e.target.value ? Number(e.target.value) : null })
            }
          />
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={program.requiresFinalExam}
              onChange={(e) =>
                upProg({
                  requiresFinalExam: e.target.checked,
                  finalExam: { ...program.finalExam, required: e.target.checked },
                })
              }
            />
            Requires final exam
          ...
          {program.requiresFinalExam && (
            <input
              ...
              value={program.finalExam.passingScore ?? ''}
              onChange={(e) =>
                upProg({
                  finalExam: { ...program.finalExam, passingScore: Number(e.target.value) },
                })
              }
            />
          )}
          {mod && (
            <div className="pt-3 border-t space-y-2">
              ...
              <input
                ...
                placeholder="Title"
                value={mod.title}
                onChange={(e) => upMod(selMod, { title: e.target.value })}
              />
              <input
                className={inp}
                placeholder="Slug"
                value={mod.slug}
                onChange={(e) => upMod(selMod, { slug: e.target.value })}
              />
              <input
                className={inp}
                placeholder="Domain key"
                value={mod.domainKey}
                onChange={(e) => upMod(selMod, { domainKey: e.target.value })}
              />
              <input
                className={inp}
                type="number"
                placeholder="Target hours"
                value={mod.targetHours}
                onChange={(e) => upMod(selMod, { targetHours: Number(e.target.value) })}
              />
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={mod.quizRequired}
                  onChange={(e) => upMod(selMod, { quizRequired: e.target.checked })}
                />{' '}
                Quiz required
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={mod.practicalRequired}
                  onChange={(e) => upMod(selMod, { practicalRequired: e.target.checked })}
                />{' '}
                Practical required
              </label>
              <button className={`${btn} w-full`} onClick={addLesson}>
                + Add Lesson
              </button>
              {mod.lessons.map((l, i) => (
                <button
        ...
        <div
          className="col-span-5 border rounded-2xl p-4 space-y-2 overflow-y-auto"
          style={{ maxHeight: '80vh' }}
        >
          ...
          {les ? (
            <>
              <input
                ...
                placeholder="Title"
                value={les.title}
                onChange={(e) => upLes(selMod, selLes, { title: e.target.value })}
              />
              <input
                className={inp}
                placeholder="Slug"
                value={les.slug}
                onChange={(e) => upLes(selMod, selLes, { slug: e.target.value })}
              />
              <select
                className={sel}
                value={les.lessonType}
                onChange={(e) => upLes(selMod, selLes, { lessonType: e.target.value as any })}
              >
                {[
                  'video',
                  'reading',
              ...
              <select
                className={sel}
                value={les.domainKey ?? ''}
                onChange={(e) => upLes(selMod, selLes, { domainKey: e.target.value || null })}
              >
              ...
              <select
                className={sel}
                value={les.hourCategory ?? ''}
                onChange={(e) =>
                  upLes(selMod, selLes, { hourCategory: (e.target.value || null) as any })
                }
              >
              ...
              <select
                className={sel}
                value={les.deliveryMethod ?? ''}
                onChange={(e) =>
                  upLes(selMod, selLes, { deliveryMethod: (e.target.value || null) as any })
                }
              >
                <option value="">Delivery method</option>
                {['online_async', 'online_live', 'in_person', 'hybrid', 'field_based'].map((dm) => (
              ...
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={!!les.practicalRequired}
                  onChange={(e) => upLes(selMod, selLes, { practicalRequired: e.target.checked })}
                />{' '}
                Practical required
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={!!les.requiresInstructorSignoff}
                  onChange={(e) =>
                    upLes(selMod, selLes, {
                      ...
                      instructorRequirement: e.target.checked
                        ? {
                            required: true,
                            approvalAuthority: 'lesson',
                            supervisionMethod: 'observation',
              ...
              <div className="flex gap-2 pt-3 border-t">
                <button className={`${btn} flex-1`} onClick={runAudit} disabled={busy}>
                  {busy ? '…' : 'Audit'}
                </button>
                <button
                  className={`${btn} flex-1 bg-black text-white disabled:opacity-40`}
                  onClick={publish}
                  disabled={busy}
                >
              ...
              <div className="text-xs text-slate-500">
                Total: {(totalMinutes / 60).toFixed(2)}h ·{' '}
                {program.modules.reduce((s, m) => s + m.lessons.length, 0)} lessons
              </div>
            </>
          ) : (
