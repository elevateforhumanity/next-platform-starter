  ...
  return (
    <tr className="hover:bg-slate-50 transition-colors">
      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">
        {s.created_at
          ? new Date(s.created_at).toLocaleDateString('en-US', { timeZone: 'UTC' })
          : '—'}
      </td>
      <td className="px-4 py-3">
        <div className="flex items-center gap-1.5">
          ...
          {s.review_status === 'flagged' && (
            <span
              title={s.flag_reason || 'Flagged for review'}
              className="inline-flex items-center gap-0.5 text-[10px] font-semibold uppercase tracking-wider bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded"
            >
              <AlertTriangle className="w-3 h-3" /> Flagged
            </span>
          )}
          {s.review_status === 'invalidated' && (
            <span className="inline-block text-[10px] font-semibold uppercase tracking-wider bg-brand-red-100 text-brand-red-700 px-1.5 py-0.5 rounded">
              Invalidated
            </span>
          )}
        </div>
        ...
        {s.is_retest && (
          <span className="inline-block mt-0.5 text-[10px] font-semibold uppercase tracking-wider bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">
            Retest
          </span>
        )}
        {s.event_count > 0 && (
          <div className="text-[10px] text-slate-400 mt-0.5">
            {s.event_count} events · {s.tab_switch_count} tab switches · {s.fullscreen_exit_count}{' '}
            FS exits
          </div>
        )}
      </td>
      ...
      <td className="px-4 py-3 text-slate-700 font-medium whitespace-nowrap">{s.exam_name}</td>
      <td className="px-4 py-3 text-slate-600 whitespace-nowrap text-xs">
        {DELIVERY_LABELS[s.delivery_method] || s.delivery_method}
      </td>
      <td className="px-4 py-3 text-center">
        {s.id_verified ? (
          <span className="inline-block w-5 h-5 bg-brand-green-100 text-brand-green-700 rounded-full text-xs leading-5 font-bold">
            ✓
          </span>
        ) : (
          <span className="inline-block w-5 h-5 bg-brand-red-100 text-brand-red-700 rounded-full text-xs leading-5 font-bold">
            ✗
          </span>
        )}
      </td>
      <td className="px-4 py-3">
        <span
          className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_COLORS[s.status] || 'bg-slate-100 text-slate-600'}`}
        >
      ...
      <td className="px-4 py-3">
        <span
          className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${RESULT_COLORS[s.result] || 'bg-slate-100 text-slate-600'}`}
        >
      ...
      <td className="px-4 py-3 text-center">
        <div className="flex items-center justify-center gap-1">
          <button
            onClick={() => onEdit(s)}
            className="p-1.5 text-slate-400 hover:text-brand-blue-600 hover:bg-brand-blue-50 rounded"
            title="Edit session"
          >
