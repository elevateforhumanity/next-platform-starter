...
export default async function AuditLogsPage() {
  ...
  const [logsRes, totalRes] = await Promise.all([
    db
      .from('audit_logs')
      .select(
        'id, action, actor_id, actor_email, actor_role, resource_type, resource_id, target_type, target_id, event_type, ip_address, created_at, metadata, details',
      )
  ...
  const actorIds = [
    ...new Set(logs.filter((l: any) => !l.actor_email && l.actor_id).map((l: any) => l.actor_id)),
  ];
    ...
    <div className="min-h-screen bg-white">
      <div className="bg-white border-b border-slate-200 px-6 py-5">
        ...
        <p className="text-sm text-slate-500 mt-1">
          {totalCount.toLocaleString()} total events · showing most recent 100
        </p>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {logs.map((log: any) => {
                  ...
                  const actorLabel =
                    log.actor_email ?? actor?.email ?? actor?.full_name ?? log.actor_id ?? 'System';
                  const resource = [
                    log.resource_type ?? log.target_type,
                    log.resource_id ?? log.target_id,
                  ]
                    .filter(Boolean)
                    .join(' ');
                  return (
                    <tr key={log.id} className="hover:bg-slate-50">
                      ...
                      <td className="py-3 px-5 text-slate-500 text-xs truncate max-w-[160px]">
                        {resource || '—'}
                      </td>
