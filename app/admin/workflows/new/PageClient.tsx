...
export default function NewWorkflowPage() {
  ...
  const setStep = (i: number, k: string, v: string) =>
    setSteps((s) => s.map((step, idx) => (idx === i ? { ...step, [k]: v } : step)));
          ...
          <div className="space-y-3">
            {steps.map((step, i) => (
              <div key={i} className="flex gap-2 items-start bg-slate-50 rounded-lg p-3">
                <span className="text-xs text-slate-400 font-mono mt-2.5 w-5 flex-shrink-0">
                  {i + 1}
                </span>
                <select
                  value={step.action}
                ...
                <input
                  value={step.config}
                  onChange={(e) => setStep(i, 'config', e.target.value)}
