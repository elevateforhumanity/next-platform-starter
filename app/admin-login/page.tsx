...
// Admin login for the main site domain.
// POSTs credentials to the admin subdomain's /api/auth/admin-login so the
// session cookie is scoped to admin.elevateforhumanity.org, then redirects there.
...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${ADMIN_URL}/api/auth/admin-login`, {
        method: 'POST',
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      // Password reset goes through the admin subdomain so the redirect URL
      // lands back on admin.elevateforhumanity.org/auth/confirm
      const res = await fetch(`${ADMIN_URL}/api/auth/forgot-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="mt-6 text-center space-y-2">
          ...
          <Link href="/login" className="block text-xs text-slate-600 hover:text-slate-400 transition-colors">
            Not an admin? Student / staff login →
          </Link>
        </div>
