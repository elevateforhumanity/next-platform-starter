      ...
      {allMessages.length === 0 ? (
        <div className="text-center py-20 bg-slate-50 rounded-2xl border border-slate-100">
          ...
          <p className="text-slate-400 text-sm mt-1">
            Messages from your mentees will appear here.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          {unread.length > 0 && (
            <section>
              ...
              <div className="space-y-2">
                {unread.map((msg: any) => (
                  <MessageRow key={msg.id} msg={msg} unread />
                ))}
              </div>
            </section>
          )}
          {read.length > 0 && (
            <section>
              ...
              <div className="space-y-2">
                {read.map((msg: any) => (
                  <MessageRow key={msg.id} msg={msg} unread={false} />
                ))}
              </div>
            </section>
          )}
...
function MessageRow({ msg, unread }: { msg: any; unread: boolean }) {
  ...
  return (
    <div
      className={`flex items-start gap-4 p-4 rounded-xl border ${unread ? 'bg-blue-50 border-blue-100' : 'bg-white border-slate-100'}`}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <p
            className={`text-sm font-semibold truncate ${unread ? 'text-slate-900' : 'text-slate-700'}`}
          >
            {name}
          </p>
        ...
        {msg.subject && (
          <p
            className={`text-sm truncate mt-0.5 ${unread ? 'font-medium text-slate-800' : 'text-slate-600'}`}
          >
            {msg.subject}
          </p>
        )}
