...
export default async function StudentPortalGradesPage() {
  ...
  const { data: trainingEnrollments } = await supabase
    ...
    .select(
      'id, progress, status, final_grade, completed_at, program:programs(title), course:training_courses(course_name)',
    )
    .eq('user_id', user.id)
  ...
  const { data: gradesRaw } = await supabase
    .from('grades')
    .select(
      'id, grade, score, max_score, status, graded_at, courses:course_id(name, code), assignments:assignment_id(title)',
    )
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl border p-5 text-center">
            ...
            <p className="text-2xl font-bold text-purple-600">
              ...
              <span className="text-sm text-slate-700">/{scores.length}</span>
            </p>
          ...
          <div className="bg-white rounded-xl border p-5 text-center">
            ...
            <p className="text-2xl font-bold text-brand-orange-600">
              {avgCheckpointScore !== null ? `${avgCheckpointScore}%` : '--'}
            </p>
        ...
        {!hasAnyData && (
          <div className="text-center py-16 border rounded-xl">
            ...
            <p className="text-slate-700 text-sm mb-4">
              Complete lessons and quizzes to see your progress here.
            </p>
            <Link
              href="/lms/courses"
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0">
                      {e.final_grade ? (
                        <span className={`text-xl font-bold ${getGradeColor(e.final_grade)}`}>
                          {e.final_grade}
                        </span>
                      ) : (
                        <span
                      ...
                      {e.completed_at && (
                        <p className="text-xs text-slate-700 mt-1">{formatDate(e.completed_at)}</p>
                      )}
                    </div>
                  </div>
                );
                  ...
                  {scores.map((s: any) => (
                    <tr key={s.id} className="border-b last:border-0">
                      ...
                      <td className="py-3 pr-4">
                        <span
                          className={`font-semibold ${s.score >= 90 ? 'text-brand-green-600' : s.score >= 70 ? 'text-brand-blue-600' : 'text-red-600'}`}
                        >
                          {s.score}%
                        </span>
                        <span className="text-slate-700 text-xs ml-1">
                          (pass: {s.passing_score}%)
                        </span>
                      </td>
                      <td className="py-3 pr-4">
                        {s.passed ? (
                          <span className="inline-flex items-center gap-1 text-brand-green-600 font-medium">
                ...
                <tbody>
                  {grades.map((g: any) => (
                    <tr key={g.id} className="border-b last:border-0">
                      <td className="py-3 pr-4 font-medium text-slate-900">
                        {g.assignments?.title ?? 'Assignment'}
                      </td>
                      <td className="py-3 pr-4 text-slate-700">
                        {g.courses?.name ?? g.courses?.code ?? '--'}
                      </td>
                      <td className="py-3 pr-4">
                      ...
                      <td className="py-3 pr-4">
                        {g.grade ? (
                          <span className={`font-semibold ${getGradeColor(g.grade)}`}>
                            {g.grade}
                          </span>
                        ) : (
                          <span className="text-slate-700">--</span>
        ...
        {progress.length > 0 && (
          <section className="rounded-xl border p-6">
            <h2 className="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
              ...
              <span className="ml-auto text-sm font-normal text-slate-700">
