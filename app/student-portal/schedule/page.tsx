...
export default async function StudentPortalSchedulePage() {
  ...
  const { data: trainingEnrollments } = await supabase
    ...
    .select(
      'id, status, cohort_id, program_id, course:training_courses(course_name, delivery_mode), program:programs(title)',
    )
    .eq('user_id', user.id)
  ...
  if (cohortIds.length > 0) {
    const { data: sessions } = await supabase
      ...
      .select(
        'id, session_date, start_time, end_time, modality, location, instructor_name, notes, cohort:cohorts(name, program:programs(title))',
      )
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="pt-6 pb-2">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <p className="text-brand-blue-600 text-xs font-bold uppercase tracking-widest mb-2">
            Student Portal
          </p>
              ...
              <div className="relative h-56 sm:h-72">
                ...
                <div className="relative h-full flex flex-col items-center justify-center text-center px-6">
                  ...
                  <p className="text-slate-600 text-sm mb-5 max-w-sm">
                    Enroll in a program to see your schedule here.
                  </p>
                  <Link
                    href="/lms/programs"
                ...
                >
                  <div className="relative h-36 overflow-hidden">
                    <Image
                      ...
                      alt={enrollment.program_title ?? 'Program'}
                      fill
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4">
                      <h3 className="font-bold text-white text-base leading-tight">
                        {enrollment.program_title ?? 'Program'}
                      </h3>
                    </div>
                    <div className="absolute top-3 right-3">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-bold ${enrollment.status === 'active' ? 'bg-brand-green-500 text-white' : 'bg-brand-blue-500 text-white'}`}
                      >
                        {enrollment.status}
                      </span>
                    </div>
                  ...
                  <div className="p-4 space-y-2">
                    {enrollment.delivery_mode && (
                      <div className="flex items-center gap-2 text-sm text-slate-700">
                        {enrollment.delivery_mode === 'online' ||
                        enrollment.delivery_mode === 'virtual' ? (
                        ...
                        <span className="capitalize">
                          {enrollment.delivery_mode.replace('_', ' ')}
                        </span>
                      </div>
                    )}
                    {enrollment.cohort_id ? (
                      <p className="text-xs text-brand-green-600 font-medium">
                        Cohort sessions scheduled — see below
                      </p>
                    ) : (
                      <p className="text-xs text-slate-400 italic">
                        No cohort assigned yet — contact student services for schedule details.
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <p className="font-semibold text-slate-900">
                        {session.program_title ?? session.cohort_name ?? 'Class Session'}
                      </p>
                    ...
                    <span
                      className={`flex-shrink-0 px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-slate-700">
                Advising sessions, orientations, and scheduled meetings
              </p>
            </div>
          </div>
            ...
            <div className="relative rounded-2xl overflow-hidden border border-gray-200">
              <div className="relative h-56 sm:h-72">
                ...
                <div className="relative h-full flex flex-col items-center justify-center text-center px-6">
                  ...
                  <p className="text-slate-600 text-sm mb-5 max-w-sm">
                    Need to speak with an advisor? Reach out to student services.
                  </p>
                  <Link
                    href="/contact"
                ...
                <div className="flex items-start gap-3 text-sm text-slate-900">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-xs mt-1">
                      Appointment-only. Confirm your session before arriving.
                    </p>
                  </div>
                </div>
