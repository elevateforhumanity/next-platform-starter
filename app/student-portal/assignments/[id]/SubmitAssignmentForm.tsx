...
export default function SubmitAssignmentForm({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (oversized.length > 0) {
      setError(
        `File(s) exceed the ${maxFileSizeMb} MB limit: ${oversized.map((f) => f.name).join(', ')}`,
      );
      return;
    }
    setError(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('step_submissions').insert({
        user_id: user.id,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Written response */}
      <div>
        <label className="block text-sm font-medium text-slate-900 mb-1">Written Response</label>
        <textarea
          value={text}
          ...
          <ul className="mt-3 space-y-2">
            {files.map((f, i) => (
              ...
              >
                ...
                <span className="text-slate-700 text-xs flex-shrink-0">
