...
export default async function StudentAssignmentDetailPage({ params }: Props) {
  ...
  const { data: assignment } = await supabase
    .from('assignments')
    .select(
      'id, title, description, instructions, due_date, max_points, allow_late_submission, late_penalty_percent, submission_type, max_file_size_mb, allowed_file_types, lesson_id, course_id',
    )
    .eq('id', id)
    .maybeSingle();
  ...
  const isLate =
    assignment.due_date &&
    submission &&
    new Date(submission.created_at) > new Date(assignment.due_date);
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <span
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold flex-shrink-0 ${statusConfig.color}`}
            >
        ...
        {submission && (
          ...
          >
            ...
            {submission.submission_text && (
              <p className="text-sm text-slate-900 whitespace-pre-wrap bg-white/60 rounded-lg p-3">
                {submission.submission_text}
              </p>
            )}
                  ...
                  {submission.file_urls.map((url: string, i: number) => (
                    <li key={i}>
                      <a
                        ...
                        className="text-sm text-brand-blue-600 hover:underline flex items-center gap-1.5"
                      >
            ...
            {submission.status === 'rejected' && (
              <div className="border-t border-red-200 pt-4">
                <p className="text-sm font-semibold text-red-700 mb-3">
                  Revision required — please resubmit below.
                </p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            ...
            <p className="text-sm text-slate-700 mb-5">
              {assignment.submission_type === 'file'
                ? 'Upload your completed work as a file.'
                : assignment.submission_type === 'text'
                  ? 'Write your response in the text box below.'
