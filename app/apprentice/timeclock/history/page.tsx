...
export default async function TimeclockHistoryPage() {
  ...
  if (byUserId) {
    ...
    if (byEmail) {
      ...
      await adminDb.from('apprentices').update({ user_id: user.id }).eq('id', byEmail.id);
    } else {
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="font-semibold text-amber-900 mb-1">
                  Account not linked to apprentice record
                </h2>
                <p className="text-amber-800 text-sm">
                  Your apprentice record exists but is not connected to your login account. Your
                  coordinator can fix this in under a minute.
                </p>
                <p className="text-amber-700 text-xs mt-3 font-mono">Your login: {user.email}</p>
                <p className="text-amber-600 text-xs mt-2">
                  Ask your coordinator to go to <strong>Admin → Apprentices → Link Account</strong>{' '}
                  and match your email.
                </p>
              </div>
            </div>
          </div>
  ...
  const approvedHours = rows
    ...
    .reduce((sum, s) => sum + s.netHours, 0);
  const pendingCount = rows.filter(
    (s) => !s.isOpen && (!s.status || s.status === 'pending'),
  ).length;
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {openCount > 0 && (
          <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800 flex items-center gap-2">
            ...
            <Link href="/apprentice/timeclock" className="underline font-medium ml-1">
              Clock out →
            </Link>
          </div>
        )}
        {autoCount > 0 && (
          <div className="mb-4 p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-600 flex items-center gap-2">
            ...
            {autoCount} shift{autoCount > 1 ? 's were' : ' was'} auto-clocked out — hours may be
            reduced.
          </div>
        )}
        ...
        {rows.length === 0 ? (
          <div className="text-center py-16 text-slate-700">
            ...
            <Link
              href="/apprentice/timeclock"
              className="mt-4 inline-block text-brand-blue-600 text-sm hover:underline"
            >
              Go to Timeclock →
            </Link>
          </div>
          ...
          <div className="space-y-8">
            {Object.entries(byMonth).map(([monthKey, monthShifts]) => {
              const monthLabel = new Date(monthKey + '-15').toLocaleDateString('en-US', {
                month: 'long',
                year: 'numeric',
              });
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-700 mt-0.5">
                                ...
                                {s.auto_clocked_out && (
                                  <span className="ml-1 text-amber-600">(auto)</span>
                                )}
                              </p>
                              ...
                              {s.hasBadData && (
                                <p className="text-xs text-red-600 mt-0.5">
                                  Clock-out before clock-in — contact coordinator
                                </p>
                              )}
                              {s.notes && (
                                <p className="text-xs text-slate-700 mt-1 italic">{s.notes}</p>
                              )}
                            </div>
                          </div>
                          <div className="text-right flex-shrink-0">
                            <p className="font-bold text-slate-900">
                              {s.isOpen ? '—' : `${s.netHours.toFixed(1)}h`}
                            </p>
                            <span
