...
export default async function StudentDashboardPage() {
  ...
  // Calculate hours
  const totalRtiMinutes = hours
    .filter((h) => h.hour_type === 'RTI')
    .reduce((sum, h) => sum + (h.minutes || 0), 0);
  const totalOjtMinutes = hours
    .filter((h) => h.hour_type === 'OJT')
    .reduce((sum, h) => sum + (h.minutes || 0), 0);
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        ...
        <nav className="md:hidden border-t border-slate-100 overflow-x-auto">
          <div className="flex px-3 py-2 gap-1 min-w-max">
            <Link
              href="/student/dashboard"
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-blue-50 text-blue-600 whitespace-nowrap"
            >
            ...
            <Link
              href="/lms/courses"
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-100 whitespace-nowrap"
            >
        ...
        <div className="bg-slate-700 rounded-xl sm:rounded-2xl p-4 sm:p-6 mb-4 sm:mb-6 text-white">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div className="min-w-0">
              <h1 className="text-lg sm:text-2xl font-bold mb-0.5 sm:mb-1 truncate">
                      ...
                      <div className="flex items-start gap-3 sm:gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-2">
                            <span
                              className={`inline-flex items-center gap-1 text-[10px] sm:text-xs px-2 py-1 sm:py-2 rounded-full ${
              ...
              <div className="bg-white rounded-xl border border-slate-200">
                ...
                <div className="divide-y divide-slate-100">
                  {hours.slice(0, 5).map((entry: any) => (
                    <div
                      key={entry.id}
                      className="flex items-center justify-between p-3 sm:p-4 gap-2"
                    >
                      <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
                        <div
                          className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full flex-shrink-0 ${entry.hour_type === 'OJT' ? 'bg-green-500' : 'bg-blue-500'}`}
                        />
                        <div className="min-w-0">
                          <p className="font-medium text-slate-900 text-xs sm:text-base truncate">
                            {entry.description || entry.hour_type}
                          </p>
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                        <span className="font-semibold text-slate-900 text-xs sm:text-base">
                          {Math.round((entry.minutes || 0) / 60)}h
                        </span>
