  ...
  const [upcoming, past, employerCount, eventCount] = await Promise.all([
    ...
    db
      ? db
          .from('events')
          .select('*', { count: 'exact', head: true })
          .in('event_type', NETWORKING_TYPES)
          .eq('is_active', true)
          .then((r) => r.count ?? 0)
      : Promise.resolve(0),
  ]);
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative h-64 sm:h-80 w-full overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 max-w-6xl mx-auto px-4 pb-8">
          ...
          <p className="text-black mt-1 text-sm max-w-xl">
            Career fairs, employer meetups, and industry networking events connecting Elevate
            graduates with hiring employers.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white py-5">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-x-10 gap-y-2 text-sm text-white">
            <span>
              <strong className="text-brand-blue-400">{eventCount}</strong> Networking Events
            </span>
            <span>
              <strong className="text-brand-blue-400">{employerCount}</strong> Employer Partners
            </span>
            <span>
              <strong className="text-brand-green-400">Free</strong> for Elevate Graduates
            </span>
          </div>
        </div>
      </div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/events" className="text-sm text-brand-blue-600 hover:underline">
            All events →
          </Link>
        </div>
        {upcoming.length === 0 ? (
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="text-black text-sm mt-1">
              Participate in our career fairs and meet pre-screened, credential-ready candidates
              from Elevate programs.
            </p>
          </div>
          <Link
