...
import { Zap, Rocket, Target, TrendingUp, Clock, Award, ArrowRight, Phone } from 'lucide-react';
  ...
  const features = [
    {
      ...
      description: 'Complete programs 50% faster with intensive study tracks',
    },
    {
      icon: Target,
      title: 'Focused Curriculum',
      description: 'Streamlined content targeting essential skills',
    },
    {
      icon: Clock,
      title: 'Flexible Scheduling',
      description: 'Evening and weekend options for working professionals',
    },
    {
      icon: Award,
      title: 'Priority Certification',
      description: 'Fast-track certification exam scheduling',
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {programs.map((program, index) => (
            <div
              key={index}
              className="bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-brand-blue-500 transition-colors"
            >
              ...
              <Link
                href={`/supersonic/programs/${program.name.toLowerCase().replace(' ', '-')}`}
                className="block text-center bg-brand-blue-600 text-white py-3 rounded-lg hover:bg-brand-blue-700 transition-colors"
              >
                Learn More
              </Link>
            </div>
