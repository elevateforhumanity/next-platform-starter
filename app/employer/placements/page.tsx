...
export default async function PlacementsPage() {
  ...
  const placementUserIds = [
    ...new Set((rawPlacements ?? []).map((p: any) => p.user_id).filter(Boolean)),
  ];
  const { data: placementProfiles } = placementUserIds.length
    ...
    : { data: [] };
  const placementProfileMap = Object.fromEntries(
    (placementProfiles ?? []).map((p: any) => [p.id, p]),
  );
  const placements = (rawPlacements ?? []).map((p: any) => ({
    ...p,
    profiles: placementProfileMap[p.user_id] ?? null,
  }));
  ...
  const { count: thisMonthCount } = await supabase
    ...
    .eq('employer_id', user.id)
    .gte('placement_date', thisMonthStart);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          <nav className="text-sm mb-4">
            <ol className="flex items-center space-x-2 text-slate-700">
              ...
              <li>/</li>
              <li className="text-slate-900 font-medium">Placements</li>
            </ol>
          </nav>
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-slate-900">Job Placements</h1>
              <p className="text-slate-700 mt-2">{count || 0} total placements</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <h3 className="text-sm font-medium text-black">Total Hires</h3>
            <p className="text-3xl font-bold text-black mt-2">{count ?? 0}</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <h3 className="text-sm font-medium text-black">This Month</h3>
            <p className="text-3xl font-bold text-black mt-2">{thisMonthCount ?? 0}</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border p-6">
            ...
            {placements && placements.length > 0 ? (
              placements.map((p: any) => (
                <div key={p.id} className="p-4 flex items-center justify-between hover:bg-white">
                  <div>
                    <p className="font-medium">{p.profiles?.full_name || 'Employee'}</p>
                    <p className="text-sm text-slate-700">
                      {p.job_title} • Started{' '}
                      {p.placement_date ? new Date(p.placement_date).toLocaleDateString() : 'N/A'}
                    </p>
                  </div>
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${p.status === 'active' ? 'bg-brand-green-100 text-brand-green-800' : 'bg-white text-slate-700'}`}
                  >
                    {p.status || 'active'}
                  </span>
                </div>
