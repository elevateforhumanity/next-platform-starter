...
CREATE POLICY employer_own_postings ON public.job_postings
  FOR ALL
  USING  (employer_id = auth.uid())
  WITH CHECK (employer_id = auth.uid());

CREATE POLICY public_active_jobs ON public.job_postings
  FOR SELECT
  USING (status = 'active');
...
CREATE POLICY applicant_own ON public.job_applications
  FOR ALL
  USING  (student_id = auth.uid())
  WITH CHECK (student_id = auth.uid());

CREATE POLICY employer_see_own_job_apps ON public.job_applications
  FOR SELECT
  USING (
    job_posting_id IN (
