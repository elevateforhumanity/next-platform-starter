...
async function main() {
  ...
  if (PRS_LESSONS.length !== 39) {
    throw new Error(
      `Expected 39 lessons, got ${PRS_LESSONS.length}. Complete all lessons before running.`,
    );
  }
  ...
  for (const m of matched) {
    const { error } = await supabase.from('curriculum_lessons').update(m.payload).eq('id', m.id);
