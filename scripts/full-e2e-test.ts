...
const RAILWAY = process.env.NEXT_PUBLIC_ADMIN_URL ?? 'https://app.elevateforhumanity.org';
...
async function main() {
  ...
  await Promise.all([
    ...
    checkUrl(roles, '/staff-portal/dashboard', `${BASE}/staff-portal/dashboard`),
    checkUrl(roles, '/staff-portal/students', `${BASE}/staff-portal/students`),
