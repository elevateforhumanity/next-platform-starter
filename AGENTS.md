...
- **Northflank admin CD:** If `elevate-admin` has `disabledCD: true`, builds succeed but pods stay on the old image. Fix: `pnpm tsx scripts/northflank/trigger-deployment.ts elevate-admin` (or enable CD via `configure-services.ts --execute`). Deploy workflow runs `trigger-deployment.ts` after every build.
