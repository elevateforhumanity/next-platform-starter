...
- **pnpm in Docker:** Store dir is **`/app/.pnpm-store`** (same filesystem as `/app/node_modules` for hardlinks). Never use `--package-import-method=copy` in Northflank Dockerfiles — it duplicates the full store into `node_modules` and triggers ENOSPC. **Do not run a separate `pnpm fetch`** on Northflank (~2GB build volume) — it fails with `ERR_PNPM_ENOSPC`. Flow: copy workspace manifests (optional layer cache), `COPY . .`, then `pnpm install --frozen-lockfile` + `pnpm store prune`. See `docs/audits/northflank-pnpm-fetch-enospc-audit.md`.
