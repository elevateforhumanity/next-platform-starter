  ...
  try {
    if (action === 'status') {
      ...
      const commits = commitsData.map((c) => ({
        ...
        when: new Date(c.commit.author.date).toLocaleString(),
      }));
      return NextResponse.json({
        branch: branchRef,
        changed: [],
    ...
    if (action === 'branches') {
      ...
      const data = (await ghFetch(`/repos/${repo}/branches?per_page=50`)) as Array<{
        name: string;
      }>;
      const branches = data.map((b) => b.name);
      return NextResponse.json({
        local: branches,
        remote: branches.map((b) => `origin/${b}`),
        current: branchRef,
        mode: 'github-api',
        repo,
      });
  ...
  try {
    ...
    if (action === 'commit') {
      ...
      git([
        ...
        '-m',
        msg,
        `--author=Admin Studio <admin@${PLATFORM_DEFAULTS.canonicalDomain}>`,
      ]);
    ...
    if (action === 'push') {
      ...
      const pushOut = ghToken
        ...
        ? git(
            [
              '-c',
              `http.https://github.com/.extraheader=Authorization: Basic ${Buffer.from(`x-access-token:${ghToken}`).toString('base64')}`,
              'push',
              'origin',
              branch,
            ],
            { timeout: 30000, token: ghToken },
          )
