...
const AdminCommandWorkbench = dynamic(
  () => import('@/components/admin/dashboard/AdminCommandWorkbench').then((m) => m.AdminCommandWorkbench),
  { ssr: false },
);
...
export default function DevStudioUnifiedClient({ isSuperAdmin = false }: { isSuperAdmin?: boolean }) {
  ...
  const previewTargets = useMemo(() => {
    const fromConfig = config?.previewTargets ?? [];
    if (fromConfig.length > 0) return fromConfig;
    const fallback = config?.defaultPreviewUrl ?? previewUrl ?? livePreviewUrl;
