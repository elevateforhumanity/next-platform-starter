...
export default function CourseBuilderClient({ initialCourses, programs }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCourse) {
        ...
        const res = await apiPatch<{ course: Course }>(
          `/api/admin/lms/courses/${editingCourse.id}`,
          {
  ...
  const handleDelete = async (courseId: string) => {
    if (
      !confirm(
        'Are you sure you want to delete this course? This will also delete all lessons and quizzes.',
      )
    )
      return;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {activeTab === 'ai' && <AutomaticCourseBuilder />}
              ...
              <div className="grid sm:grid-cols-2 gap-4 flex-1 text-sm">
                <div>
                  ...
                  <p className="text-slate-600 text-xs leading-relaxed">
                    Title, description, outcomes, credentials, funding tracks, compliance profile,
                    CTAs. Managed in{' '}
                    <Link href="/admin/programs" className="text-brand-blue-600 hover:underline">
                      Programs
                    </Link>
                    .
                  </p>
          ...
          <div className="bg-white rounded-lg shadow-sm border">
            ...
            {filteredCourses.length > 0 ? (
                  ...
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${course.status === 'published' ? 'bg-brand-green-100 text-brand-green-700' : 'bg-brand-orange-100 text-brand-orange-700'}`}
                        >
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-sm text-brand-red-600 border border-brand-red-200 rounded-lg hover:bg-brand-red-50"
                      >
                        Delete
                      </button>
                    </div>
              ...
              <div className="px-6 py-12 text-center">
                ...
                <button onClick={openCreateModal} className="text-brand-blue-600 hover:underline">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, publish_immediately: e.target.checked })
                      }
                      className="w-4 h-4 rounded"
                    />
                    <label htmlFor="publish_immediately" className="text-sm text-slate-900">
                      Publish immediately
                    </label>
                  </div>
