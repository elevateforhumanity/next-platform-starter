  ...
  return (
    <div className="p-6 grid grid-cols-12 gap-4 min-h-screen text-sm">
      ...
      <div className="col-span-3 border rounded-2xl p-4 space-y-2">
        ...
        <button className="border rounded px-3 py-1.5 w-full text-xs" onClick={addModule}>
          + Add Module
        </button>
      ...
      >
        ...
        {program.requiresFinalExam && (
          <input
            className={inp}
            type="number"
            placeholder="Final exam passing score"
            value={program.finalExam.passingScore ?? ''}
            onChange={(e) =>
              upProg({ finalExam: { ...program.finalExam, passingScore: Number(e.target.value) } })
            }
          />
        )}
        ...
        {mod && (
          <div className="pt-3 border-t space-y-2">
            ...
            <button className="border rounded px-3 py-1.5 w-full text-xs" onClick={addLesson}>
              + Add Lesson
            </button>
            {mod.lessons.map((l, i) => (
              <button
      ...
      >
        ...
        {les ? (
          <>
            ...
            <select
              className={sel}
              value={les.evidenceType ?? ''}
              onChange={(e) =>
                upLes(selMod, selLes, { evidenceType: (e.target.value || null) as any })
              }
            >
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                className="border rounded px-4 py-2 flex-1 bg-black text-white disabled:opacity-40"
                onClick={publish}
                disabled={busy}
              >
                {busy ? '…' : 'Publish'}
