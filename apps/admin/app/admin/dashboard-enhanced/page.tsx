...
export default async function DashboardEnhancedPage() {
  ...
  const passRate = totalQuizAttempts
    ? Math.round(((passedQuizzes ?? 0) / totalQuizAttempts) * 100)
    : 0;
  ...
  const STATS = [
    ...
    },
    {
      ...
      sub: 'lab & assignment submissions',
      icon: AlertCircle,
      color: (pendingSubmissions ?? 0) > 0 ? 'text-red-500' : 'text-green-500',
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              ...
              <Link href="/admin/curriculum" className="text-sm text-brand-red-600 hover:underline">
                Manage
              </Link>
            </div>
            {!courses?.length ? (
              ...
              <div className="divide-y divide-slate-100">
                {courses.map((c: any) => (
                  <div key={c.id} className="flex items-center justify-between px-6 py-3">
                    ...
                    <span
                      className={`text-xs font-semibold px-2.5 py-1 rounded-full ${c.status === 'published' ? 'bg-green-50 text-green-700' : c.status === 'draft' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-500'}`}
                    >
                      {c.status}
                    </span>
                  </div>
