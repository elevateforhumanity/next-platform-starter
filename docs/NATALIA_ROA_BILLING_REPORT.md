# Natalia Roa - Barber Apprenticeship Billing Report
**Generated:** June 18, 2026  
**Email:** natataroa@gmail.com  
**Student ID:** ELV-2026-00263  
**Portal Type:** Barber

---

## Program Enrollment

| Field | Value |
|-------|-------|
| Program | Barber Apprenticeship |
| Enrollment Status | Active |
| Enrolled Date | May 3, 2026 |
| Funding Source | Self-Pay |

---

## Billing Summary

| Metric | Amount |
|--------|--------|
| **Full Tuition** | $4,980.00 |
| **Initial Payment (Setup Fee)** | $600.00 |
| **Remaining Balance** | $4,077.94 |
| **Weekly Payment** | $151.03 |
| **Weeks Remaining** | 48 |

### Payment Status
- **Status:** Active ✅
- **Auto-Renewal:** Enabled (Stripe)
- **Stripe Customer ID:** cus_UTVa6pmsYlWBsp
- **Stripe Subscription ID:** sub_1TiizwH4a2yrVOt5NVcCV61S
- **Payment Model:** Stripe Auto

---

## Hours Analysis ⚠️ DISCREPANCY FOUND

### Expected vs Actual Hours

**Enrolled:** May 3, 2026  
**Today:** June 18, 2026  
**Weeks Elapsed:** 7 weeks  
**Expected Hours (40/week):** 280 hours  
**Expected Hours (thru last complete week):** 240 hours

### Hours Currently Logged

| Date | Hours | Category | Status |
|------|-------|----------|--------|
| May 12, 2026 | 40 | Shop Floor | Approved ✅ |
| May 23, 2026 | 40 | Shop Floor | Approved ✅ |
| June 5, 2026 | 40 | On-the-Job | Approved ✅ |
| **TOTAL LOGGED** | **120 hours** | | |

### ❌ MISSING: 120 HOURS

**Weeks NOT Logged:**
- May 3-9, 2026 (Week 1)
- May 10-16, 2026 (Week 2) ← overlap with logged entry
- May 17-23, 2026 (Week 3) ← overlap
- May 24-30, 2026 (Week 4)
- May 31-June 6, 2026 (Week 5) ← overlap
- June 7-13, 2026 (Week 6)
- June 14-18, 2026 (Week 7 - current week, OK to exclude)

### ✅ SUCCESSFUL PAYMENTS (6)

| Date | Amount |
|------|--------|
| May 7, 2026 | $600.00 |
| May 12, 2026 | $151.03 |
| May 19, 2026 | $151.03 |
| May 26, 2026 | $151.03 |
| June 1, 2026 | $151.03 |
| June 2, 2026 | $151.03 |
| **TOTAL** | **$1,255.12** |

## ❌ FAILED PAYMENTS (6)

| Date | Amount | Reason |
|------|--------|--------|
| May 7, 2026 | $600.00 | incorrect_zip |
| June 9, 2026 | $151.03 | incorrect_zip |
| June 11, 2026 | $151.03 | incorrect_zip |
| June 15, 2026 | $151.03 | incorrect_zip |
| June 15, 2026 | $151.03 | incorrect_zip |
| June 16, 2026 | $151.03 | incorrect_zip |
| **TOTAL** | **$1,053.06** | |

## 💰 PAYMENT SUMMARY

| | |
|---|---|
| Total Successful | $1,255.12 |
| Total Failed | $1,053.06 |
| **Net Received** | **$1,255.12** |

## ✅ SUBSCRIPTION STATUS

| | |
|---|---|
| Stripe Subscription | **CANCELED** |
| Cancel at period end | true |
| Reason | Payment failed - ZIP code issue |

---

## Action Items

### 1. Hours Discrepancy - NEEDS ATTENTION
- **Logged:** 120 hours
- **Expected:** 240 hours (6 complete weeks)
- **Missing:** 120 hours
- **Possible Causes:**
  - Hours not properly entered
  - Hours in different table/database
  - Batch import missing

### 2. Check Stripe for Pending Payments
Login to https://dashboard.stripe.com and search:
- Email: natataroa@gmail.com
- Customer ID: cus_UTVa6pmsYlWBsp
- Subscription: sub_1TiizwH4a2yrVOt5NVcCV61S

---

## Stripe Dashboard Access

- Customer ID: cus_UTVa6pmsYlWBsp
- Subscription: sub_1TiizwH4a2yrVOt5NVcCV61S
